...
const FEATURES = [
  "Генерация рецептов в чате без ограничений",
  "План питания на неделю",
  "Учёт аллергий и предпочтений",
  "Замена блюд",
] as const;

function trialDaysPhrase(days: number): string {
...
/**
 * Модалка сразу после успешной активации trial. Визуально близка к UnifiedPaywall, без блока оплаты.
 */
export function TrialActivatedModal({ open, userId, onClose }: TrialActivatedModalProps) {
  const dismiss = () => {
    markTrialActivatedModalSeen(userId);
    onClose();
  };

  const subtitle = `У вас есть ${trialDaysPhrase(TRIAL_DURATION_DAYS)} полного доступа ко всем функциям`;
            ...
            <div className="flex flex-col min-h-0 flex-1 overflow-hidden">
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pt-2.5 pb-1.5 sm:px-5 sm:pt-3 sm:pb-2 pr-11 space-y-2">
                ...
                <div className="text-center space-y-1 px-0.5">
                  ...
                  <p className="text-sm text-muted-foreground leading-snug text-balance">{subtitle}</p>
                </div>
