...
Deno.test("generateRecipeSystemPromptV3: содержит RECIPE_SYSTEM_RULES_V3", () => {
  ...
  if (!prompt.includes("аллергии и dislikes запрещены")) {
    throw new Error("Expected prompt to contain allergies/dislikes rule");
  }
  if (!prompt.includes("mealType (breakfast|lunch|dinner|snack)")) {
    throw new Error("Expected prompt to contain mealType constraint");
  }
});
