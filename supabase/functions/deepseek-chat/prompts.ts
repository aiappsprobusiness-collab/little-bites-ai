...
{
  "title": string,
  "description": string (110–190 симв., макс. 210; ровно 2 предложения о пользе),
...
ОПИСАНИЕ (польза блюда): ровно 2 коротких предложения, 110–190 символов, макс. 210. Предложение 1 — основная польза блюда. Предложение 2 — 1–2 конкретных нутритивных акцента от ключевых ингредиентов (белок, клетчатка, железо, кальций, витамин C, бета-каротин, медленные углеводы, полезные жиры, сытость, поддержка энергии, пищеварение, кости, мышцы). Не начинать с названия, не дублировать title, не писать «это блюдо», «в составе», «главный ингредиент», «подходит для», про хранение/подачу/текстуру как главную мысль. Не перечислять ингредиенты списком. Запрещено: «отличный выбор для разнообразия рациона», «приятная текстура», «насыщенный вкус», «простое блюдо», «подходит для всей семьи», «идеально подходит», «получается ароматным», «сытное, но не тяжёлое», «в составе», обрывы предложения. Пример: «Куриная грудка даёт полноценный белок для сытости и поддержки мышц. Киноа добавляет клетчатку и железо, томаты — витамин C для усвоения.»
...
Поля: title, description (110–190 симв., макс. 210, ровно 2 предложения о пользе), ingredients [{name, amount}] до 10, steps 5–7 (до 150 симв. каждая), cookingTime, mealType, servings, chefAdvice (140–260 симв., 2–3 предложения), nutrition (kcal_per_serving, protein/fat/carbs, is_estimate: true).
mealType только: breakfast|lunch|dinner|snack.
description — польза: ровно 2 предложения, 110–190 симв. Предложение 1 — основная польза. Предложение 2 — 1–2 нутритивных акцента (белок, клетчатка, железо, кальций, витамин C, бета-каротин, медленные углеводы, сытость, энергия). Не начинать с названия, не «это блюдо»/«в составе»/«подходит для»/хранение/подача/«приятная текстура»/«разнообразие рациона». Не перечислять ингредиенты. Завершать оба предложения точкой.
...
Обращение только «Вы»: в steps и chefAdvice — формы на «Вы» (натрите, добавьте, подавайте, смешайте, очистите); запрещено «ты» и формы на «ты» (натри, помой, очисть, смешай, добавь).
