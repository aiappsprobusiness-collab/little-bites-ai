...
/** Единый контракт рецепта. Stage 2.4: LLM description — основной источник; при невалидном система подставит fallback. */
...
{
  "title": string,
  "description": string (1–2 предложения, макс. 160 симв.; не повторять название блюда),
  "ingredients": [ { "name": string, "amount": string } ] (макс. 10),
  "steps": string[] (5–7 шагов, макс. 150 символов на шаг),
  "cookingTime": number,
  "mealType": "breakfast" | "lunch" | "dinner" | "snack",
  "servings": number,
  "chefAdvice": string | null (0–2 коротких предложения, макс. 220 симв.; только если есть конкретная польза по технике/таймингу/текстуре этого блюда; иначе null),
...
ОПИСАНИЕ: 1–2 предложения, макс. 160 симв. Не повторять название блюда в начале. Запрещено: «в дорогу», «для ребёнка», «для всей семьи», контекст профиля. Без медицинских обещаний (лечит, излечивает и т.п.).
СОВЕТ ОТ ШЕФА (chefAdvice): либо null, либо 1–2 коротких предложения, макс. 220 символов. Только если совет реально помогает приготовить именно это блюдо (температура, время, текстура, порядок ингредиентов, типичная ошибка для этого рецепта). Если сильного совета нет — верни null; не заполняй поле ради формы. Запрещены общие фразы без привязки к рецепту: «подавайте сразу», «не пережаривайте» без конкретики, «свежие продукты», «специи по вкусу», «украсьте зеленью», «для аромата специи» и т.п. Не дублируй дословно шаги. Без восклицательных знаков и пафосных двоеточий в начале. Хорошо: «Кабачок отожмите перед фаршем — котлеты не расползутся при жарке.» Плохо: шаблон, который подойдёт к любому блюду. Запрещено начинать: «Для максимальной…», «Это блюдо», «Данное блюдо», «Совет:».
...
/**
 * V3: компактные правила для recipe-path. Stage 2.4: LLM description — основной источник (1–2 предложения, макс. 160 симв.).
 */
export const RECIPE_SYSTEM_RULES_V3 = `
Верни ровно 1 JSON-объект рецепта. Без текста до/после, без markdown.
Поля: title, description (1–2 предложения, макс. 160 симв.), ingredients [{name, amount}] до 10, steps 5–7 (до 150 симв. каждая), cookingTime, mealType, servings, chefAdvice (null или 1–2 предложения, макс. 220; только конкретный совет по блюду), nutrition (kcal_per_serving, protein/fat/carbs, is_estimate: true).
mealType только: breakfast|lunch|dinner|snack.
description: 1–2 предложения, макс. 160 симв. Не повторять название блюда. Не писать «в дорогу», «для ребёнка», «для всей семьи». Без медицинских обещаний.
chefAdvice: null или 1–2 предложения, макс. 220 симв. Только практический совет по этому блюду; если нечего ценного сказать кратко — null. Без универсальных заглушек и без дубля шагов. Запрещено: «Для максимальной…», «Это блюдо», пустой тон «подавайте тёплым» без контекста. Пример: «Отожмите кабачок перед фаршем — фрикадельки держат форму при жарке.»
