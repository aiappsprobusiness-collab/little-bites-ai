...
/**
 * Полные правила поля chefAdvice в JSON рецепта. Синхронизировать с chefAdviceQuality.ts (длина, одно предложение).
 */
export const CHEF_ADVICE_RULES = `
Ты даёшь короткий совет шефа для конкретного рецепта.

Цель:
дать ОДИН полезный, конкретный и неочевидный инсайт, который реально улучшает блюдо.
...
СТРОГО ЗАПРЕЩЕНО:
- советы уровня "используйте свежие продукты"
- банальные кулинарные истины
- общие рекомендации без привязки к рецепту
- начинать с "Для более...", "Чтобы...", "Если хотите..."
- давать несколько советов в одном тексте
- повторять шаги рецепта

ОБЯЗАТЕЛЬНО:
- совет должен быть привязан к конкретному ингредиенту или шагу рецепта
- совет должен давать микро-технику, лайфхак или нюанс
- совет должен звучать как от шефа, а не как из статьи

ХОРОШИЕ ПРИМЕРЫ:
- Добавьте щепотку сахара к томатам — это уберёт лишнюю кислотность и усилит вкус.
- Слегка разомните часть фасоли в супе — бульон станет гуще без сливок.
- Обжарьте специи 30 секунд перед добавлением жидкости — аромат станет ярче.

ПЛОХИЕ ПРИМЕРЫ (ЗАПРЕЩЕНО):
- Используйте свежие продукты
- Для лучшего вкуса добавьте специи
- Чтобы блюдо было вкуснее, готовьте на среднем огне

Если нет действительно полезного инсайта — верни null.
`.trim();

/** Единый контракт рецепта. Канон recipes.description: LLM при прохождении gate, иначе buildRecipeBenefitDescription (deepseek-chat pickCanonicalDescription). */
...
{
  "title": string,
  "description": string (1–2 коротких предложения, макс. 160 симв.; живой человеческий тон, без канцелярита),
  "ingredients": [ { "name": string, "amount": string } ] (макс. 10),
  "steps": string[] (5–7 шагов, макс. 150 символов на шаг),
  "cookingTime": number,
  "mealType": "breakfast" | "lunch" | "dinner" | "snack",
  "servings": number,
  "chefAdvice": string | null (одно предложение, макс. ~140 симв.; строго по правилам [CHEF_ADVICE] ниже; иначе null или ""),
...
ОПИСАНИЕ (короткий текст для карточки и для сохранения в БД): 1–2 предложения, макс. 160 симв. Живой, аппетитный тон — хочется готовить; без канцелярита и без длинного абзаца. Не дублировать title почти дословно и не начинать с названия блюда. Запрещены штампы: «отличный вариант», «полезное и вкусное блюдо», «подходит для всей семьи», «идеально сочетается» и т.п. Без медицинских обещаний. Не писать «в дорогу», «с собой», «в контейнер», «для ребёнка», «для всей семьи» без явного запроса пользователя. Хотя бы одно предложение с нутритивным акцентом (белок, клетчатка, железо, сытость, энергия, пищеварение и т.д.).
...
Поля: title, description (1–2 коротких предложения, макс. 160 симв.; живой аппетитный тон), ingredients [{name, amount}] до 10, steps 5–7 (до 150 симв. каждая), cookingTime, mealType, servings, chefAdvice (null или одно предложение по правилам ниже), nutrition (kcal_per_serving, protein/fat/carbs, is_estimate: true).
mealType только: breakfast|lunch|dinner|snack.
description: подзаголовок к блюду — хочется приготовить; штампы и мед. обещания запрещены (CONTRACT). Нутритивный акцент в одном из предложений.
