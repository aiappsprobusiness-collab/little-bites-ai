import { CHEF_ADVICE_MAX_LENGTH } from "./domain/recipe_io/chefAdviceQuality.ts";
import {
 ...
 * Legacy для шаблонов Free/Premium (non-recipe). Не входит в `generateRecipeSystemPromptV3`; лимиты `description` для JSON recipe-path — в RECIPE_SYSTEM_RULES_V3.
 */
...
/**
 * Правила chefAdvice для промпта. Детальный отсев — в chefAdviceQuality.ts / enforceChefAdvice (null без заглушек).
 */
export const CHEF_ADVICE_RULES = `
chefAdvice: ровно одно предложение, до ${CHEF_ADVICE_MAX_LENGTH} симв., без «!».
Один неочевидный практический нюанс: микро-техника или лайфхак, привязанный к ингредиенту или шагу этого рецепта (не общие истины).
Запрет: «свежие продукты», «специи по вкусу», несколько советов в одном тексте, дословный повтор шагов, вода без техники.
Вводные «Для более…», «Чтобы…», «Если хотите…» — только если дальше конкретное действие и эффект по рецепту; иначе null.
Нет сильного инсайта — null.
`.trim();

/**
 * Справочный полный контракт (тесты, доки). Живой recipe-path собирает промпт через `generateRecipeSystemPromptV3` + `RECIPE_SYSTEM_RULES_V3`, без вставки этой константы целиком.
 * Длина description — как `passesDescriptionQualityGate` (константы DESCRIPTION_* в sanitizeAndRepair.ts); макс. 210 = серверный лимит, не 160.
 */
...

{
  ...
  "servings": number,
  "chefAdvice": string | null (одно предложение, до ${CHEF_ADVICE_MAX_LENGTH} симв.; правила [CHEF_ADVICE] ниже; иначе null),
...
Обращение только «Вы» в steps и chefAdvice (императив «Добавьте/Обжарьте» — норма). Без «!» в chefAdvice. Не дублируй шаги дословно.
...
Если выбран профиль СЕМЬЯ: один рецепт, подходящий и взрослым, и ребёнку. Формат — только валидный JSON как в [RECIPE TASK]. chefAdvice: только про приготовление блюда или null; без возраста и адаптаций.
`;
...
Likes — необязательная подсказка, не требование. Не используй любимый продукт как главную основу в каждом рецепте подряд; чередуй белки и «героев» блюда. Если пользователь не просил конкретно этот продукт — чаще выбирай другие уместные варианты.`;
