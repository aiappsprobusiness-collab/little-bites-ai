...
/** Единый контракт рецепта: короткие правила для description и chefAdvice (без длинных примеров). */
...
{
  "title": string,
  "description": string (до 170 символов; 1–2 коротких предложения),
  "ingredients": [ { "name": string, "amount": string } ] (макс. 10),
  "steps": string[] (5–7 шагов, макс. 150 символов на шаг),
  "cookingTime": number,
  "mealType": "breakfast" | "lunch" | "dinner" | "snack",
  "servings": number,
  "chefAdvice": string (до 280 символов; 2–3 коротких предложения),
...
ОПИСАНИЕ: 1–2 коротких предложения. Не повторять название блюда. Сказать, что это за блюдо + один конкретный плюс. Без общих фраз, без хранения/подачи/разогрева, без штампов («идеально подходит», «приятный вкус», «универсальный», «сбалансированное», «это блюдо», «подходит для всей семьи», «простое в приготовлении блюдо», «в составе»).
СОВЕТ ОТ ШЕФА: 2–3 коротких предложения. Только конкретная кулинарная польза для этого блюда (текстура, аромат, температура, обжарка/запекание, порядок ингредиентов, подача). Запрещены заходы: «Для максимальной сочности», «Для более нежной текстуры», «Для получения…», «Если хотите», «Вы можете», «Это блюдо», «Данное блюдо», «Чтобы блюдо получилось», «Это позволит». Лучше короткий живой совет, чем длинный и общий.
...
Строго: аллергии и dislikes запрещены. Если запрос конфликтует — подбери замену и верни рецепт. Не отказывай.
Возраст: <12 мес — без соли, сахара, мёда, коровьего молока; мягкая текстура. 12–35 мес — без жарки/острого, безопасные кусочки.
Не меняй mealType. Ингредиенты: у каждого amount с единицей (г, мл, шт., ст.л., ч.л.).
`;
