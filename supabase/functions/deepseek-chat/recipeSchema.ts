 ...
 * description: Zod max 210 = `DESCRIPTION_MAX_LENGTH` (sanitizeAndRepair); минимальная приемлемая длина для канона LLM — `DESCRIPTION_QUALITY_MIN_LENGTH` (38), правила предложений, мягкий title-anchoring и запрет утечки адаптации профиля/аллергий (`descriptionHasProfileAdaptationLeak` → `explainCanonicalDescriptionRejection`: `profile_adaptation_leak`) в `passesDescriptionQualityGate` (тот же модуль).
