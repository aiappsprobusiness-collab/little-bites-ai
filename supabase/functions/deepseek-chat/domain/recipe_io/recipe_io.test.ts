...
import {
  ...
  buildEmergencyChatRecipeDescription,
} from "./index.ts";
...
Deno.test("descriptionHasProfileAdaptationLeak: «без молока» / «без яиц» / формулировки адаптации", () => {
  const leaks = [
    "Нежное мясо и кремовое пюре без молока. Белок и клетчатка для сытости.",
    "Омлет без яиц с овощами. Клетчатка и белок для энергии.",
    "Вариант подходит при аллергии на глютен. Сытость и энергия.",
    "Блюдо адаптировано для тех, кто не ест рыбу. Полезные жиры и белок.",
  ];
  for (const s of leaks) {
    if (!descriptionHasProfileAdaptationLeak(s)) {
      throw new Error(`Expected leak detected: ${s.slice(0, 50)}…`);
    }
  }
});

Deno.test("descriptionHasProfileAdaptationLeak: обычные «без» (майонез, лишней тяжести) — не утечка", () => {
  const ok = [
    "Свежие овощи с клетчаткой и витамином C. Заправка на йогурте без майонеза — легче для пищеварения.",
    "Каша даёт энергию без лишней тяжести. Белок и медленные углеводы для сытости.",
  ];
  for (const s of ok) {
    if (descriptionHasProfileAdaptationLeak(s)) {
      throw new Error(`Expected no adaptation leak: ${s.slice(0, 60)}…`);
    }
  }
});

Deno.test("passesDescriptionQualityGate: «без молока» → отказ, explain → profile_adaptation_leak", () => {
  const desc =
    "Нежное мясо с овощами и пюре без молока. Богато белком и клетчаткой для сытного ужина.";
  if (passesDescriptionQualityGate(desc, { title: "Тушёная баранина с овощами" })) {
    throw new Error("Expected gate fail for profile adaptation in description");
  }
  const reason = explainCanonicalDescriptionRejection(desc, { title: "Тушёная баранина с овощами" });
  if (reason !== "profile_adaptation_leak") {
    throw new Error(`Expected profile_adaptation_leak, got ${reason}`);
  }
});

Deno.test("pickCanonicalDescription: утечка адаптации профиля → deterministic_fallback + profile_adaptation_leak", () => {
  const fallback = "Канон benefit для БД без маркеров аллергии.";
  const llm =
    "Нежное мясо с мягкими овощами и кремовым пюре без молока. Богато белком и клетчаткой для сытного ужина.";
  const pick = pickCanonicalDescription({
    sanitizedLlmDescription: llm,
    title: "Тушёная баранина с овощами",
    deterministicFallback: fallback,
  });
  if (pick.source !== "deterministic_fallback" || pick.description !== fallback) {
    throw new Error(`Expected fallback, got: ${JSON.stringify(pick)}`);
  }
  if (pick.rejectionReason !== "profile_adaptation_leak") {
    throw new Error(`Expected profile_adaptation_leak, got ${pick.rejectionReason}`);
  }
});

Deno.test("buildDescriptionFallback: детерминированные шаблоны не содержат profile adaptation leak", () => {
  const seeds = ["a", "b", "c", "seed-family", "x1", "y2"];
  for (const seed of seeds) {
    for (const title of ["Суп", "Котлеты", "Тушёная баранина", "Овсянка с ягодами"]) {
      const d = buildDescriptionFallback({ title, recipeIdSeed: seed });
      if (descriptionHasProfileAdaptationLeak(d)) {
        throw new Error(`Fallback must not contain adaptation leak: ${d}`);
      }
    }
  }
});

Deno.test("enforceDescription: «без глютена» в тексте → заменяется на benefit fallback", () => {
  const bad =
    "Сочные котлеты без глютена из гречки. Белок и клетчатка для сытости и энергии на весь день.";
  const out = enforceDescription(bad, { title: "Котлеты", recipeIdSeed: "gluten-test" });
  if (descriptionHasProfileAdaptationLeak(out)) {
    throw new Error(`Sanitized description must not leak profile adaptation: ${out}`);
  }
});
...
Deno.test("passesDescriptionQualityGate: два предложения короче порога — отказ (too_short_for_two_sentences)", () => {
  const twoButShort = "Сытный обед с белком. Клетчатка в овощах.";
  if (twoButShort.length < DESCRIPTION_QUALITY_MIN_LENGTH) {
    throw new Error(`Fixture must be >= ${DESCRIPTION_QUALITY_MIN_LENGTH}, got ${twoButShort.length}`);
  }
...
Deno.test("passesDescriptionQualityGate: сенсорика и бытовая польза проходят при длинном title (ослабленный anchoring)", () => {
  const title = "Лосось с киноа и брокколи на пару";
  const desc =
    "Нежная кремовая текстура тает во рту. Деликатный аромат и сочная сердцевина держат аппетит ровно без тяжести.";
  if (desc.length < DESCRIPTION_QUALITY_MIN_LENGTH) {
    throw new Error(`Fixture must be >= ${DESCRIPTION_QUALITY_MIN_LENGTH}`);
  }
  if (!passesDescriptionQualityGate(desc, { title })) {
    throw new Error(`Expected pass, got ${explainCanonicalDescriptionRejection(desc, { title })}`);
  }
});

Deno.test("buildEmergencyChatRecipeDescription: нет типичных маркетинговых штампов", () => {
  for (const seed of ["a", "b", "c", "x9"]) {
    const d = buildEmergencyChatRecipeDescription({
      title: "Ужин с курицей",
      ingredients: ["Курица", "Рис"],
      recipeIdSeed: seed,
    });
    if (
      /это блюдо|этот вариант|идеально подходит|отличный выбор|хорошо вписывается|легко вписывается|подходит для всей семьи/i.test(
        d,
      )
    ) {
      throw new Error(`Emergency text must avoid marketing stamps: ${d}`);
    }
    if (d.length > DESCRIPTION_MAX_LENGTH || d.length < 20) {
      throw new Error(`Bad emergency length: ${d.length}`);
    }
  }
});

Deno.test("description title anchoring: generic «белок и клетчатка» без привязки к лососю → missing_title_anchoring", () => {
  const title = "Лосось с киноа и брокколи на пару";
  const generic =
    "Сбалансированный ужин с высококачественным белком и клетчаткой. Легко усваивается и обеспечивает длительное чувство сытости.";
  if (passesDescriptionQualityGate(generic, { title })) {
    throw new Error("Expected generic description to fail gate");
  }
  const reason = explainCanonicalDescriptionRejection(generic, { title });
  if (reason !== "missing_title_anchoring") {
    throw new Error(`Expected missing_title_anchoring, got ${reason}`);
  }
});

Deno.test("description title anchoring: Том Ям с креветками + конкретика в тексте → проходит", () => {
  const title = "Тайский суп Том Ям с креветками";
  const desc =
    "Острый суп на кокосовом молоке с лаймом и креветками. Белок морепродуктов и полезные жиры поддерживают сытость.";
  if (!passesDescriptionQualityGate(desc, { title })) {
    throw new Error(`Expected pass, got ${explainCanonicalDescriptionRejection(desc, { title })}`);
  }
});

Deno.test("description title anchoring: короткое название (2 сильных токена) → fail-open, каша без слова овсянка проходит", () => {
  const title = "Овсянка с изюмом";
  const strong = extractStrongTitleTokensForDescriptionAnchoring(title);
  if (strong.length >= DESCRIPTION_TITLE_ANCHOR_MIN_STRONG_TOKENS) {
    throw new Error("Fixture must have <4 strong tokens for fail-open");
  }
  const borderline =
    "Каша на молоке сытная: белок и клетчатка для энергии дня.";
  if (!descriptionPassesTitleAnchoringHeuristic(borderline, title)) {
    throw new Error("Fail-open: anchoring must pass for <3 strong title tokens");
  }
  if (!passesDescriptionQualityGate(borderline, { title })) {
    throw new Error(`Expected full gate pass, got ${explainCanonicalDescriptionRejection(borderline, { title })}`);
  }
});

Deno.test("pickCanonicalDescription: generic ужин → deterministic_fallback (missing_title_anchoring)", () => {
  const fallback = "Канон benefit для БД.";
  const llm =
    "Сбалансированный ужин с белком и клетчаткой. Легко усваивается и даёт длительное чувство сытости.";
  const pick = pickCanonicalDescription({
    sanitizedLlmDescription: llm,
    title: "Лосось с киноа и брокколи на пару",
    deterministicFallback: fallback,
  });
  if (pick.source !== "deterministic_fallback" || pick.rejectionReason !== "missing_title_anchoring") {
    throw new Error(`Expected fallback missing_title_anchoring, got ${JSON.stringify(pick)}`);
  }
  if (pick.description !== fallback) {
    throw new Error("Expected fallback description text");
  }
});
