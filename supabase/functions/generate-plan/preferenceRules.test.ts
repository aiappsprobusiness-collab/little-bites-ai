...
Deno.test("passesPreferenceFilters blocks nut allergy for recipe with орехи in ingredients", () => {
  const allowed = passesPreferenceFilters(
    {
  ...
  if (allowed) {
    throw new Error("Expected nut allergy to block recipe with орехи in ingredients");
  }
});

Deno.test("passesPreferenceFilters allows nut allergy when орех only in title", () => {
  const allowed = passesPreferenceFilters(
    {
      title: "Творожная запеканка с орехами",
      description: "Нежный завтрак",
      recipe_ingredients: [{ name: "творог" }, { name: "банан" }, { name: "яйцо" }],
    },
    { allergies: ["орехи"] },
  );
  if (!allowed) {
    throw new Error("Title-only «орехами» must not block on nut allergy (ingredient-only match)");
  }
});

Deno.test("passesPreferenceFilters allows gluten allergy for творожная запеканка without gluten ingredients", () => {
  const allowed = passesPreferenceFilters(
    {
  ...
  if (!allowed) {
    throw new Error("Творожная запеканка must pass gluten allergy when ingredients are gluten-free");
  }
});
    ...
    passesPreferenceFilters(
      {
        ...
        description: "",
        recipe_ingredients: [{ name: "куриное филе" }],
      },
      { allergies: ["мясо"] },
    )
  ) {
