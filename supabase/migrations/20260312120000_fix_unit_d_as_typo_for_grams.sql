-- Исправление опечатки «д» вместо «г» (граммы).
-- В списке покупок отображались «15 д», «1200 д» — подмена кириллической «г» на «д».

-- 1) normalize_ingredient_unit: трактовать «д» как граммы
...
BEGIN
  ...
  u := lower(trim(unit));
  u := regexp_replace(u, '\.$', '');
  IF u IN ('г', 'гр', 'g', 'грамм', 'д') THEN RETURN 'g'; END IF;
...
-- 2) create_recipe_with_steps: при сохранении подменять unit «д» на «г», если canonical = g
...
COMMENT ON FUNCTION public.create_recipe_with_steps(jsonb) IS 'Creates recipe + steps + ingredients. Safe numeric casts. Fixes unit typo д -> г when canonical is g.';

-- 3) Исправить уже сохранённые данные
UPDATE public.recipe_ingredients
SET unit = 'г', display_text = replace(replace(display_text, ' д ', ' г '), ' д', ' г')
WHERE btrim(COALESCE(unit, '')) = 'д';
