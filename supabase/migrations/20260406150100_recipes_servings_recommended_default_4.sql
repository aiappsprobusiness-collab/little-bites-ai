-- Default UX: servings_recommended 4 when omitted in payload; backfill legacy 1/NULL.

ALTER TABLE public.recipes ALTER COLUMN servings_recommended SET DEFAULT 4;

UPDATE public.recipes SET servings_recommended = 4, updated_at = now()
WHERE servings_recommended IS NULL OR servings_recommended = 1;
...
BEGIN
  ...
  INSERT INTO public.recipes (
    ...
    GREATEST(1, COALESCE((payload->>'servings_recommended')::integer, 4)),
    v_locale,
    v_source_lang,
    v_trust_level,
    v_nutrition_goals,
...
COMMENT ON FUNCTION public.create_recipe_with_steps(jsonb) IS
  'Creates recipe + steps + ingredients. Default servings_recommended in DB/RPC when omitted: 4. Optional per-ingredient: display_amount, display_unit, display_quantity_text, measurement_mode (canonical_only|dual|display_only). Dual without quantity text requires amount+unit.';
