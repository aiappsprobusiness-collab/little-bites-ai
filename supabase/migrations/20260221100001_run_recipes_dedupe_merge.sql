-- Reusable dedupe: run_recipes_dedupe_merge(p_dry_run). Idempotent.
...
BEGIN
  -- Build mapping loser_id -> winner_id (one winner per loser; if loser in several groups, pick one)
  ...
  -- === APPLY: redirect refs, then archive + delete ===

  -- 1) favorites_v2: loser -> winner; avoid duplicate (user_id, recipe_id) by deleting row if user already has winner
  ...
  -- 2) meal_plans_v2.meals: replace loser_id with winner_id in each slot
  FOR plan_rec IN
  ...
  -- 3) chat_history.recipe_id (if column exists)
  IF EXISTS (
  ...
  -- 4) Archive losers to trash (no delete without archive)
  ...
  -- 6) Safety: sanity-check — битые ссылки после apply (должно быть 0)
...
COMMENT ON FUNCTION public.run_recipes_dedupe_merge(boolean) IS 'Cleanup-only dedupe: redirect refs, archive losers to trash, delete. p_dry_run=true by default; real apply only on explicit run_recipes_dedupe_merge(false). No anti-duplicate guard on insert. Idempotent.';
