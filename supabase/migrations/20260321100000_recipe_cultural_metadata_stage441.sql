-- Stage 4.4.1: cultural metadata on recipes (cuisine, region, familiarity) + infer helper.
-- Lists mirror supabase/functions/_shared/inferCulturalFamiliarity.ts — keep in sync.

ALTER TABLE public.recipes
  ADD COLUMN IF NOT EXISTS cuisine text NULL,
  ADD COLUMN IF NOT EXISTS region text NULL,
  ADD COLUMN IF NOT EXISTS familiarity text NULL;

ALTER TABLE public.recipes
  DROP CONSTRAINT IF EXISTS recipes_familiarity_check;

ALTER TABLE public.recipes
  ADD CONSTRAINT recipes_familiarity_check CHECK (
    familiarity IS NULL OR familiarity IN ('classic', 'adapted', 'specific')
  );

COMMENT ON COLUMN public.recipes.cuisine IS
  'Stage 4.4: coarse cuisine label (string slug); not derived from locale. Foundation for pool ranking.';

COMMENT ON COLUMN public.recipes.region IS
  'Stage 4.4: optional regional hint; no normalized taxonomy on MVP.';

COMMENT ON COLUMN public.recipes.familiarity IS
  'Stage 4.4: classic | adapted | specific — cultural breadth for future generate-plan soft scoring; inferred if omitted on insert.';

-- Deterministic inference; mirrors TS inferCulturalFamiliarity (see _shared).
CREATE OR REPLACE FUNCTION public.infer_cultural_familiarity(p_cuisine text)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
...
DECLARE
  k text;
BEGIN
  IF p_cuisine IS NULL OR btrim(p_cuisine) = '' THEN
    RETURN 'adapted';
  END IF;
  k := lower(regexp_replace(btrim(p_cuisine), '\s+', '_', 'g'));

  IF k IN (
    'neutral', 'common', 'international', 'everyday', 'home', 'family', 'default',
    'local_default', 'default_local', 'generic', 'simple', 'comfort', 'home_cooking',
    'russian', 'ukrainian', 'belarusian', 'post_soviet', 'cis'
  ) THEN
    RETURN 'classic';
  END IF;

  IF k IN (
    'ethiopian', 'eritrean', 'somali', 'nepalese', 'bhutanese', 'laotian', 'mongolian',
    'niche', 'regional_ethnic_specialty', 'deeply_regional', 'indigenous', 'ethnic_niche'
  ) THEN
    RETURN 'specific';
  END IF;

  IF k IN (
    'italian', 'american', 'french', 'mexican', 'chinese', 'japanese', 'indian',
    'thai', 'vietnamese', 'korean', 'spanish', 'greek', 'turkish', 'lebanese',
    'mediterranean', 'british', 'german', 'polish', 'persian', 'georgian', 'armenian',
    'azerbaijani', 'uzbek', 'israeli', 'moroccan', 'tunisian', 'egyptian', 'syrian',
    'iraqi', 'iranian', 'pakistani', 'bangladeshi', 'indonesian', 'malaysian', 'filipino',
    'brazilian', 'peruvian', 'cuban', 'caribbean', 'scandinavian', 'dutch', 'austrian',
    'swiss', 'portuguese', 'australian', 'canadian', 'fusion', 'middle_eastern', 'asian',
    'european', 'latin_american', 'african'
  ) THEN
    RETURN 'adapted';
  END IF;
...
COMMENT ON FUNCTION public.infer_cultural_familiarity(text) IS
  'Stage 4.4.1: maps cuisine slug to classic|adapted|specific; no locale. Keep lists aligned with inferCulturalFamiliarity.ts.';
...
COMMENT ON FUNCTION public.create_recipe_with_steps(jsonb) IS
  'Creates recipe + steps + ingredients. Stage 4.4.1: optional cuisine, region, familiarity; familiarity inferred via infer_cultural_familiarity when omitted.';
