-- Auto-fill tags for user_custom recipes: ["user_custom"] and optionally ["user_custom", "user_custom_${meal_type}"].
-- If p_tags is NULL or empty, compute in RPC; otherwise use p_tags as-is. Chat_ai and other sources unchanged.

-- ========== create_user_recipe: auto tags when p_tags null/empty ==========
...
BEGIN
  ...
  -- Normalize meal_type for tags: trim, lower, NULL if empty; only allow breakfast/lunch/dinner/snack/other
  ...
  INSERT INTO public.recipes (
    ...
    'user_custom',
    NULLIF(btrim(COALESCE(p_meal_type, '')), ''),
    v_tags,
    NULLIF(btrim(COALESCE(p_chef_advice, '')), ''),
    NULL,
    v_uid,
    'private'
  )
...
COMMENT ON FUNCTION public.create_user_recipe(text, text, text, text[], text, jsonb, jsonb) IS 'Creates private user_custom recipe. Tags: if p_tags null/empty then ["user_custom"] or ["user_custom","user_custom_${meal_type}"].';

-- ========== update_user_recipe: auto tags when p_tags null/empty ==========
...
BEGIN
  ...
  -- Same tags logic as create_user_recipe
  v_meal_type_norm := NULLIF(lower(btrim(COALESCE(p_meal_type, ''))), '');
  IF v_meal_type_norm IS NOT NULL AND v_meal_type_norm NOT IN ('breakfast','lunch','dinner','snack','other') THEN
    v_meal_type_norm := NULL;
  END IF;
  ...
  SET
    ...
    meal_type = NULLIF(btrim(COALESCE(p_meal_type, '')), ''),
    tags = v_tags,
    chef_advice = NULLIF(btrim(COALESCE(p_chef_advice, '')), ''),
    updated_at = now()
...
COMMENT ON FUNCTION public.update_user_recipe(uuid, text, text, text, text[], text, jsonb, jsonb) IS 'Updates user_custom recipe. Tags: if p_tags null/empty then ["user_custom"] or ["user_custom","user_custom_${meal_type}"].';
