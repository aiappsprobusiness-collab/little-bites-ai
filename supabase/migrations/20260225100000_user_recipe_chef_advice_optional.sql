-- Make chef_advice optional for user_custom recipes (create_user_recipe / update_user_recipe).
-- p_chef_advice accepts NULL; empty string is stored as NULL. recipes.chef_advice already allows NULL.

-- ========== create_user_recipe: p_chef_advice nullable, no chef_advice_required ==========
...
COMMENT ON FUNCTION public.create_user_recipe(text, text, text, text[], text, jsonb, jsonb) IS 'Creates private user_custom recipe + steps + ingredients. chef_advice optional (NULL if empty).';

-- ========== update_user_recipe: p_chef_advice nullable, no chef_advice_required ==========
...
COMMENT ON FUNCTION public.update_user_recipe(uuid, text, text, text, text[], text, jsonb, jsonb) IS 'Updates user_custom recipe and replaces steps/ingredients. chef_advice optional (NULL if empty).';
