-- user_custom: meal_type только breakfast | lunch | snack | dinner (как слоты плана).
-- Убираем поддержку 'other' и любых прочих строк в RPC + чистим данные.

UPDATE public.recipes
SET meal_type = NULL
WHERE meal_type IS NOT NULL
  AND lower(btrim(meal_type)) NOT IN ('breakfast', 'lunch', 'dinner', 'snack');

UPDATE public.recipes
SET tags = array_remove(tags, 'user_custom_other'::text)
WHERE tags IS NOT NULL
  AND 'user_custom_other'::text = ANY (tags);
...
COMMENT ON FUNCTION public.create_user_recipe(text, text, text, text[], text, jsonb, jsonb) IS
  'Creates private user_custom recipe + steps + ingredients. meal_type only breakfast|lunch|dinner|snack or NULL; tags auto when p_tags empty.';

COMMENT ON FUNCTION public.update_user_recipe(uuid, text, text, text, text[], text, jsonb, jsonb) IS
  'Updates user_custom recipe. meal_type only breakfast|lunch|dinner|snack or NULL; tags auto when p_tags empty.';
