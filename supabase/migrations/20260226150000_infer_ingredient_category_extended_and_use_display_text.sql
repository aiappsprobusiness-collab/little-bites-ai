-- 1) Расширить infer_ingredient_category: больше русских основ/синонимов, порядок проверок сохранён.
--    Используется при записи (create_recipe_with_steps и т.д.) с аргументом name + display_text.
...
DECLARE
  n text;
BEGIN
  IF name_clean IS NULL OR btrim(name_clean) = '' THEN
    RETURN 'other'::public.product_category;
  END IF;
  n := lower(btrim(name_clean));

  -- meat: расширено (окорок, грудинка, шея, фарш, котлета, колбас и т.д. в контексте мяса)
  IF n ~ '(говядин|свинин|баранин|индейк|куриц|фарш|котлет|телятин|окорок|грудинк|шея|колбас|сосиск|бекон|ветчин)' THEN RETURN 'meat'::public.product_category; END IF;
  -- fish
  IF n ~ '(рыба|лосос|треск|тунец|семг|форел|карп|судак|минтай|сельд|скумбр|кальмар|креветк|краб|икра)' THEN RETURN 'fish'::public.product_category; END IF;
  -- dairy
  IF n ~ '(молок|кефир|йогурт|творог|сыр|сметан|сливк|ряженк|простокваш)' THEN RETURN 'dairy'::public.product_category; END IF;
  -- grains: паста, лапша, хлеб
  IF n ~ '(круп|овсян|греч|рис|макарон|паста|мука|лапш|хлеб|сухар|булгур|киноа|перлов)' THEN RETURN 'grains'::public.product_category; END IF;
  -- vegetables: расширено
  IF n ~ '(морков|кабач|тыкв|капуст|картоф|лук|огурц|помидор|перец|баклажан|горох|фасол|чеснок|сельдер|шпинат|салат|редис|свекл|редиск|броккол|цветн|капуст|зелен)' THEN RETURN 'vegetables'::public.product_category; END IF;
  -- fruits
  IF n ~ '(яблок|банан|груш|ягод|клубник|черник|малин|виноград|слив|абрикос|персик|манго|апельсин|лимон|мандарин|киви|авокадо|гранат|инжир)' THEN RETURN 'fruits'::public.product_category; END IF;
  -- fats: масло растительное/оливковое/сливочное
  IF n ~ '(масло|оливк|сливочн|подсолнечн|растительн)' THEN RETURN 'fats'::public.product_category; END IF;
  -- spices
  IF n ~ '(соль|перец|специи|укроп|петруш|базилик|кинза|кориандр|лавр|гвоздик|кориц|имбир|чеснок|паприк|орегано|тимьян|мята)' THEN RETURN 'spices'::public.product_category; END IF;

  RETURN 'other'::public.product_category;
END;
$$;

COMMENT ON FUNCTION public.infer_ingredient_category(text) IS 'Category from name/display_text (Russian stems). Call with name || '' '' || display_text for best results.';

-- 2) create_recipe_with_steps: передавать в infer объединённые name + display_text
...
COMMENT ON FUNCTION public.create_recipe_with_steps(jsonb) IS 'Creates recipe + steps + ingredients. Infers category from name+display_text. Uses servings_base and servings_recommended from payload (default 1).';
