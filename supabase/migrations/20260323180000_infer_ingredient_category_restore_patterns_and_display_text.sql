-- Восстановление качества категорий ингредиентов:
-- 1) infer_ingredient_category: после 20260313130000 осталась урезанная версия без многих стемов
--    (куриное филе ≠ «куриц», свиная вырезка ≠ «свинин», минтай/киноа/авокадо и т.д.).
-- 2) create_recipe_with_steps: снова передаём в infer name || display_text (как в 20260313120000),
--    а не только final_name.
-- 3) Backfill для существующих строк с category = other.
...
BEGIN
  ...
  -- meat: курин (куриное/куриный), свин (свиная), говяж; сосиски/ветчина; яйц (яйцо)
  IF n ~ '(говядин|говяж|свинин|свин|баранин|индейк|курин|куриц|фарш|котлет|телятин|окорок|грудинк|шея|колбас|сосиск|бекон|ветчин|яйц)' THEN
    RETURN 'meat'::public.product_category;
  END IF;
  IF n ~ '(рыба|лосос|треск|тунец|семг|форел|карп|судак|минтай|сельд|скумбр|кальмар|креветк|краб|икра)' THEN
    RETURN 'fish'::public.product_category;
  END IF;
  -- dairy: сливочн до fats (масло сливочное — не fats)
  IF n ~ '(молок|кефир|йогурт|творог|сыр|сметан|сливк|сливочн|ряженк|простокваш)' THEN
    RETURN 'dairy'::public.product_category;
  END IF;
  IF n ~ '(круп|овсян|греч|рис|макарон|паста|мука|лапш|хлеб|сухар|булгур|киноа|перлов|чечевиц|нут|манн|пшён)' THEN
    RETURN 'grains'::public.product_category;
  END IF;
  IF n ~ '(морков|кабач|тыкв|капуст|картоф|лук|огур|огурц|помидор|томат|перец|баклажан|горох|горош|фасол|чеснок|сельдер|шпинат|салат|редис|свекл|редиск|броккол|цветн|зелен|цукин|гриб|шампиньон|спарж|тофу|хумус|овощн|руккол)' THEN
    RETURN 'vegetables'::public.product_category;
  END IF;
  IF n ~ '(яблок|банан|груш|ягод|клубник|черник|малин|виноград|слив|абрикос|персик|манго|апельсин|лимон|лайм|мандарин|киви|авокадо|гранат|инжир|изюм|финик)' THEN
    RETURN 'fruits'::public.product_category;
  END IF;
  IF n ~ '(масло|оливк|растительн)' THEN
    RETURN 'fats'::public.product_category;
  END IF;
  IF n ~ '(соль|перец|специи|укроп|петруш|базилик|кинза|кориандр|лавр|гвоздик|кориц|имбир|чеснок|паприк|орегано|тимьян|мята)' THEN
...
COMMENT ON FUNCTION public.infer_ingredient_category(text) IS
  'Категория по name и/или name+display_text (русские основы). Вызывать с btrim(name || '' '' || display_text) для лучшего распознавания.';
...
COMMENT ON FUNCTION public.create_recipe_with_steps(jsonb) IS
  'Creates recipe + steps + ingredients. Категория при other/null: infer_ingredient_category(name || display_text). Stage 4.4.1 cultural metadata.';
...
WHERE ri.category = 'other'::public.product_category
  AND public.infer_ingredient_category(btrim(COALESCE(ri.name, '') || ' ' || COALESCE(ri.display_text, ''))) IS DISTINCT FROM 'other'::public.product_category;
