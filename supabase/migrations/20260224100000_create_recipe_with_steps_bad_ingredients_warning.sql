-- create_recipe_with_steps: add diagnostic WARNING when some ingredients look "bad"
-- (display_text null/short, name empty, amount+canonical both null/zero, unit+canonical both empty).
-- Does not block creation; only RAISE WARNING for logs.
...
BEGIN
  ...
  LOOP
    ...
    -- Diagnostic: count "bad" ingredients (do not block insert)
...
COMMENT ON FUNCTION public.create_recipe_with_steps(jsonb) IS 'Creates recipe + steps + ingredients. Logs WARNING when some ingredients have missing/short display_text, empty name, or no amount/unit/canonical.';
