-- get_recipe_full: разрешить полное чтение рецептов из пула (как get_recipe_previews).
-- План/прикорм подставляет recipe_id каталоговых seed-рецептов (user_id каталога);
-- превью уже отдавались через get_recipe_previews, а экран рецепта — нет → «Рецепт не найден».
...
COMMENT ON FUNCTION public.get_recipe_full(uuid, text) IS
  'Full recipe for detail screen. Access: own recipes, user_custom by owner, or pool sources (seed/starter/manual/week_ai/chat_ai) for any authenticated user — aligned with get_recipe_previews.';
