-- Stage 4: nutrition_goals in recipes + create_recipe_with_steps support.

ALTER TABLE public.recipes
  ADD COLUMN IF NOT EXISTS nutrition_goals jsonb NOT NULL DEFAULT '[]'::jsonb;

ALTER TABLE public.recipes
  DROP CONSTRAINT IF EXISTS recipes_nutrition_goals_check;

ALTER TABLE public.recipes
  ADD CONSTRAINT recipes_nutrition_goals_check CHECK (
    jsonb_typeof(nutrition_goals) = 'array'
    AND NOT jsonb_path_exists(nutrition_goals, '$[*] ? (@.type() != "string")')
    AND NOT jsonb_path_exists(
      nutrition_goals,
      '$[*] ? (@ != "balanced" && @ != "iron_support" && @ != "brain_development" && @ != "weight_gain" && @ != "gentle_digestion" && @ != "energy_boost")'
    )
  );

COMMENT ON COLUMN public.recipes.nutrition_goals IS
  'Stage 4 goals: balanced, iron_support, brain_development, weight_gain, gentle_digestion, energy_boost.';
...
COMMENT ON FUNCTION public.create_recipe_with_steps(jsonb) IS
  'Creates recipe + steps + ingredients. Stage 4: accepts nutrition_goals jsonb array with fixed whitelist.';
