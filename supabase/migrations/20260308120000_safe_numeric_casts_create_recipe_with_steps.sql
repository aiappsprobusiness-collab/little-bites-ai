-- Safe numeric casts in create_recipe_with_steps: avoid 22P02 when payload has
-- amount/canonical_amount as display strings (e.g. "258 г") instead of numeric-only.
...
COMMENT ON FUNCTION public.create_recipe_with_steps(jsonb) IS 'Creates recipe + steps + ingredients. Safe numeric casts for amount/canonical_amount/order_index (avoids 22P02).';
