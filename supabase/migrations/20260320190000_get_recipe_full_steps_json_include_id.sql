-- steps_json: добавить id шага; step_number и порядок остаются из recipe_steps (canonical).
-- recipe_step_translations не хранит step_number; локализуется только instruction.
...
COMMENT ON FUNCTION public.get_recipe_full(uuid, text) IS
  'Full recipe for detail screen. steps_json: id and step_number from recipe_steps (canonical); instruction localized via recipe_step_translations with fallback. Steps ordered by step_number ASC.';
