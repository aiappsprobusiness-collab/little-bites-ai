-- Remove image_url from create_recipe_with_steps: column recipes.image_url was dropped.
-- RPC must not reference it or INSERT fails with 42703.
...
COMMENT ON FUNCTION public.create_recipe_with_steps(jsonb) IS 'Creates recipe + steps + ingredients. Uses servings_base and servings_recommended from payload (default 1). Column image_url removed.';
