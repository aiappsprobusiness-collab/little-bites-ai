-- Stage 5: новые product-события + platform из properties (клиент getAnalyticsPlatform).
-- DROP required: CREATE OR REPLACE VIEW cannot insert/rename columns mid-list vs prior view.
...
COMMENT ON VIEW analytics.usage_events_enriched IS
  'Обогащённые usage_events: event_group, event_type, jsonb properties как плоские поля. platform из properties (web|pwa|ios|android|unknown). Stage 5: recipe_view, share_link_created, replace attempt/fail.';
