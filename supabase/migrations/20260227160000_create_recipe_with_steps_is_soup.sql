-- create_recipe_with_steps: принимать is_soup из payload и записывать в recipes.is_soup.
-- assign_recipe_to_plan_slot не трогает recipes (в т.ч. meal_type, is_soup) — только meal_plans_v2.meals.
...
BEGIN
  ...
  v_is_soup := COALESCE((payload->>'is_soup')::text IN ('true', 't', '1', 'yes'), false);
...
COMMENT ON FUNCTION public.create_recipe_with_steps(jsonb) IS 'Creates recipe + steps + ingredients. Accepts is_soup in payload. Assigning recipe to plan slot (assign_recipe_to_plan_slot) does not update recipes.meal_type or recipes.is_soup.';
