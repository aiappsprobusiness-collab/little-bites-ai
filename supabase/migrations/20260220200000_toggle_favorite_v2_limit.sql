-- toggle_favorite_v2: RPC for add/remove favorite with Free limit 15. Returns { ok, is_favorite? } or { ok: false, code, limit? }.
...
BEGIN
  ...
  IF NOT v_is_premium THEN
    ...
    IF v_count >= 15 THEN
      RETURN jsonb_build_object('ok', false, 'code', 'favorites_limit_reached', 'limit', 15);
    END IF;
  END IF;
...
COMMENT ON FUNCTION public.toggle_favorite_v2(uuid, uuid, jsonb) IS 'Toggle favorite; Free limit 15. Returns { ok, is_favorite } or { ok: false, code: favorites_limit_reached, limit: 15 }.';
