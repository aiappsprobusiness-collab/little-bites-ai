-- =============================================================================
-- Когорты по неделе первого auth_success (Stage 4)
-- =============================================================================
-- cohort_week: date_trunc('week', first_auth_date)::date (UTC Monday week start в PG по ISO)
-- Размер когорты: DISTINCT user_id с первым auth в эту неделю (в окне наблюдения).
-- activation: как funnel_activation (chat_recipe, plan_fill_day, favorite_add,
--   plan_slot_replace_success, plan_fill_day_success, help, recipe_view) после first_auth_at.
-- median time to activation: секунды от first_auth до первой активации.
-- paywall reach: был ли paywall_view после first_auth (в пределах data_through).
-- purchase reach: запись в subscription_plan_audit (любая дата до data_through).
-- =============================================================================
...
cohort AS (
  ...
  FROM first_auth fa
  CROSS JOIN params p
  WHERE fa.first_auth_date >= p.cohort_from
    AND fa.first_auth_date < p.cohort_to
),
activation AS (
  SELECT
    c.user_id,
...
tta AS (
  SELECT
    a.signup_week,
    EXTRACT(EPOCH FROM (a.first_activation_at - c.first_auth_at)) AS sec
  FROM activation a
...
SELECT
  ...
  ROUND((SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY t.sec)
         FROM tta t WHERE t.signup_week = cs.signup_week)::numeric, 0) AS median_seconds_to_activation,
