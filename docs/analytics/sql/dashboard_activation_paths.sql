-- =============================================================================
-- Dashboard: Activation paths (обзор для BI)
-- =============================================================================
-- Согласовано с activation_path_breakdown.sql (path_bucket, median time, D7 proxy).
-- Параметры: cohort_from, cohort_to (UTC date), data_through (UTC date) — как в Stage 4.
-- =============================================================================
...
activity_d7 AS (
  ...
  INNER JOIN paths pth ON pth.user_id = ue.user_id
  CROSS JOIN params p
...
SELECT
  pth.path_bucket,
  COUNT(*) AS activated_users_in_cohort_window,
  ROUND(100.0 * COUNT(*) / SUM(COUNT(*)) OVER (), 2) AS pct_of_activated,
  ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY pth.sec_to_activation)::numeric, 0) AS median_sec_to_activation,
  COUNT(*) FILTER (WHERE (pth.cohort_date + 7) <= pr.data_through) AS eligible_d7,
  COUNT(*) FILTER (
    WHERE (pth.cohort_date + 7) <= pr.data_through AND a7.user_id IS NOT NULL
  ) AS retained_d7,
  ROUND(
    100.0 * COUNT(*) FILTER (
      WHERE (pth.cohort_date + 7) <= pr.data_through AND a7.user_id IS NOT NULL
    ) / NULLIF(COUNT(*) FILTER (WHERE (pth.cohort_date + 7) <= pr.data_through), 0),
    2
  ) AS retention_d7_pct_proxy
FROM paths pth
CROSS JOIN params pr
LEFT JOIN activity_d7 a7 ON a7.user_id = pth.user_id
GROUP BY pth.path_bucket, pr.data_through
ORDER BY activated_users_in_cohort_window DESC;
