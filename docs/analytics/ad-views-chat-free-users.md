...
- В `src/utils/subscriptionRules.ts`: у free задано `aiDailyLimit: 5` (5 ИИ-запросов в день в режиме рецептов).
- На бэкенде в `deepseek-chat/index.ts` для типа `chat`/`recipe` у не-premium пользователя проверяется лимит по той же фиче `chat_recipe` и константе `FREE_AI_DAILY_LIMIT` (5): если `used >=` лимита, возвращается 429 и запрос не выполняется. После успешной генерации в `usage_events` пишется событие с `feature: "chat_recipe"`.

Итого: **реклама не увеличивает лимит** — она обязательный шаг перед **второй и каждой следующей** генерацией в тот же день (`usedToday >= 1`); дневной лимит — **5** подборов (`usage_events.chat_recipe`, `FREE_AI_DAILY_LIMIT`).
...
Таким образом, просмотры рекламы для фри-пользователей во вкладке чат реализованы как обязательный шаг перед второй (и последующей в тот же день) генерацией в режиме «Рецепты», с учётом лимита **5** подборов в день по `usage_events.chat_recipe` и stub-модалкой с аналитикой `ad_rewarded_*`.
