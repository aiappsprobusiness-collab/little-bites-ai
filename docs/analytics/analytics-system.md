...
4. **План дня/недели:** `/p/:ref` → `shared_plan_view` или `shared_plan_not_found_view` → CTA `share_day_plan_cta_click` / `share_week_plan_cta_click` → `/auth?mode=signup&entry_point=shared_*_plan&share_ref=...` (см. [ANALYTICS_EVENT_TAXONOMY_STAGE2.md](../decisions/ANALYTICS_EVENT_TAXONOMY_STAGE2.md) §3.10).
5. **signup_from_share:** после `auth_success` на `AuthPage` / `TelegramStartPage`, если в localStorage/onboarding атрибуция `share_recipe` / `shared_recipe` / `share_type=recipe` и известен `recipe_id` (или резолв по `share_refs`) — RPC `record_recipe_feedback(..., signup_from_share)`.
