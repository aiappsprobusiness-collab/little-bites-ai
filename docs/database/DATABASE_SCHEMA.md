...
**RPC get_public_catalog_recipe_by_id(p_recipe_id, p_locale)** — то же по структуре ответа, для публичного тизера `/recipe/teaser/:id` по UUID рецепта из каталожного пула (без `user_custom`). SECURITY DEFINER, доступен anon.
...
- **get_public_catalog_recipe_by_id(p_recipe_id, p_locale text DEFAULT NULL)** — тот же JSON-формат, что у **get_recipe_by_share_ref**, но по `recipes.id` и только для каталожного пула: `source IN ('seed', 'starter', 'manual', 'week_ai', 'chat_ai')`. **SECURITY DEFINER**, `GRANT EXECUTE` для **anon** и **authenticated**; публичная страница `/recipe/teaser/:id` (например из Telegram) без share_ref.
