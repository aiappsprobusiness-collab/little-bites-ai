...
**Лимиты (триггер `members_enforce_subscription_limits_trigger`, `members_enforce_subscription_limits`):** по `profiles_v2.premium_until` / `trial_until` — если оплаченный или активный trial доступ есть: не более **7** строк `members` на пользователя, не более **7** активных аллергий, **5** likes и **5** dislikes на строку; иначе (free): не более **1** профиля и **1** активной аллергии. Дублирует продуктовые лимиты из `src/utils/subscriptionRules.ts`.
