...
- **get_recipe_by_share_ref(p_share_ref, p_locale text DEFAULT NULL)** — рецепт по короткой ссылке шаринга для публичной страницы (anon). При p_locale — title, description, chef_advice и steps/ingredients с fallback на переводы по локали. Объекты **ingredients** включают те же поля measurement/dual, что и **ingredients_json** в get_recipe_full.
