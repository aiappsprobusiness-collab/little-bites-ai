...
| trust_level         | text                | Уровень доверия пула: seed, starter, trusted, candidate, blocked. CHECK: только эти значения или NULL. Пул (generate-plan) исключает blocked. |
...
UNIQUE(recipe_id, locale). Индекс по (recipe_id, locale). Триггер updated_at. RLS включён; прямого SELECT для anon/authenticated нет — доступ только через SECURITY DEFINER RPC. Шаги и ингредиенты на этом этапе не локализуются (recipe_steps / recipe_ingredients без переводов).
...
- **create_recipe_with_steps(payload)** — создание рецепта из чата/недели (рецепт + шаги + ингредиенты с нормализацией). В payload опционально: locale (по умолчанию 'ru'), source_lang, trust_level (по умолчанию по source: seed/starter → одноимённый, manual → trusted, chat_ai/week_ai → candidate).
...
- **recompute_recipe_score_and_trust(p_recipe_id)** — пересчёт score (формула выше, clamp [-10, 50]) и trust: для trusted только score, без auto-block; candidate → trusted при score≥8, likes≥2, dislikes≤1; candidate → blocked при (dislikes≥4 или score≤−6) и total_votes≥3 (cold start: при <3 голосах не блокировать). Ручной UPDATE trust_level допустим. Вызывается триггерами INSERT/DELETE на recipe_feedback.
- **get_recipe_previews(recipe_ids, p_locale text DEFAULT NULL)** — превью рецептов (в т.ч. is_favorite из favorites_v2). При переданном p_locale подставляются title/description из recipe_translations с fallback на recipes.
- **get_recipe_full(p_recipe_id, p_locale text DEFAULT NULL)** — полный рецепт со шагами и избранным. При p_locale — title, description, chef_advice из recipe_translations с fallback на recipes; шаги/ингредиенты пока в одной локали.
...
- **has_recipe_translation(p_recipe_id, p_locale)** — ML-5 hardening: true, если у рецепта уже есть запись в recipe_translations для локали (проверяет владельца). Используется Edge translate-recipe для пропуска вызова LLM при существующем переводе.
- **get_recipe_by_share_ref(p_share_ref)** — рецепт по короткой ссылке шаринга для публичной страницы (anon).
