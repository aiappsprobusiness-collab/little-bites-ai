...
**Истечение подписки в строке профиля:** при `trial_until <= now()` или при `premium_until <= now()` (если `premium_until` задан) статус сбрасывается в `free`, `daily_limit = 5`. Email из внутреннего whitelist (`is_internal_unlimited_email`, зеркало `UNLIMITED_ACCESS_EMAILS` в `src/utils/subscriptionAccess.ts`) при истёкшем `premium_until` **не** сбрасывается. RPC `expire_profile_subscription_if_needed()` (authenticated, без аргументов) — при загрузке профиля в `useSubscription`; Edge `deepseek-chat` / `generate-plan` — тот же сброс при запросе. Пакетно: `expire_stale_subscription_profiles_batch()` (service_role). Эффективный тариф в UI и на Edge — `resolveEffectiveSubscription` (`premium_until` / `trial_until` / whitelist), не только `status`.
...
### `public.telegram_onboarding_sessions`

Состояние pre-auth Telegram onboarding-бота (edge webhook `telegram-onboarding`): шаги анкеты и собранные ответы до перехода в веб-регистрацию.
...
| step          | text    | idle \| await_age \| await_allergies \| await_likes \| await_dislikes \| done |
| status        | text    | active \| completed \| cancelled |
| age_months    | integer | NULL или 6..216 |
| allergies     | text[]  | Нормализованные аллергии |
| likes         | text[]  | Нормализованные likes-теги |
| dislikes      | text[]  | Нормализованные dislikes-теги |
| utm           | jsonb   | UTM/blogger attribution для CTA |
| prompt_message_id | bigint | `message_id` последнего сообщения с inline-клавиатурой (чипы), для `editMessageReplyMarkup` |
| menu_example_delivered | boolean NOT NULL DEFAULT false | После успешной первой выдачи превью меню — true; следующие финалы без генерации меню |
| created_at, updated_at, last_event_at | timestamptz | Временные метки состояния |

RLS: включён; запись/чтение выполняет Edge Function через `service_role`.
...
Статьи (legacy): таблица сохранена, **клиентский UI снят** (2026-05). RLS без изменений.
...
**RPC get_public_catalog_recipe_by_id(p_recipe_id, p_locale)** — то же по структуре ответа, для публичного тизера **`/t/:id`** по UUID рецепта из каталожного пула (без `user_custom`). SECURITY DEFINER, доступен anon.
...
### `public.telegram_bloggers`

Справочник блогеров для ссылок **`t.me/<bot>?start=...`**: поле `code` совпадает с `blogger_id` в query внутри `start` (атрибуция в `usage_events` после прохождения бота). Управление — админ-страница `/admin/telegram-blogger-links` при `VITE_ADMIN_MODE=true` (см. `docs/dev/TELEGRAM_BLOGGER_LINKS.md`).

| Колонка      | Тип     | Описание |
|--------------|---------|----------|
| id           | uuid PK | |
| code         | text NOT NULL UNIQUE | Короткий код, **lower case**, `^[a-z0-9_]{1,32}$` (CHECK) |
| display_name | text NOT NULL | Подпись для себя (блогер, канал) |
| channel_url  | text NULL | Ссылка на канал (опционально) |
| is_active    | boolean NOT NULL DEFAULT true | Можно отключить, не удаляя запись |
| notes        | text NULL | Внутренние заметки |
| created_at   | timestamptz NOT NULL | |
| updated_at   | timestamptz NOT NULL | Триггер `update_updated_at_column` на UPDATE |

RLS: SELECT / INSERT / UPDATE / DELETE — `anon`, `authenticated` (тот же операционный риск, что у `marketing_links` — ссылка админки не должна быть публичной). Миграция: `20260428120000_telegram_bloggers.sql`.
...
- **get_public_catalog_recipe_by_id(p_recipe_id, p_locale text DEFAULT NULL)** — тот же JSON-формат, что у **get_recipe_by_share_ref**, но по `recipes.id` и только для каталожного пула: `source IN ('seed', 'starter', 'manual', 'week_ai', 'chat_ai')`. **SECURITY DEFINER**, `GRANT EXECUTE` для **anon** и **authenticated**; публичная страница **`/t/:id`** (и редирект с легаси `/recipe/teaser/:id`), например из Telegram, без share_ref.
