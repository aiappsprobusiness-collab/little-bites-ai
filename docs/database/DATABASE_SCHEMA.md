...
- **recompute_recipe_score_and_trust(p_recipe_id)** — пересчёт score (формула выше, clamp [-10, 50]) и trust: для trusted только score, без auto-block; candidate → trusted при score≥8, likes≥2, dislikes≤1; candidate → blocked при dislikes≥4 или score≤−6. Ручной UPDATE trust_level допустим. Вызывается триггерами INSERT/DELETE на recipe_feedback.
