...
- **Обновление не автоматическое:** новый worker переходит в `waiting`. При `reg.waiting && navigator.serviceWorker.controller` диспатчится `sw-update-available`. Компонент **`PWAUpdateToast`**: по «Обновить» — `SKIP_WAITING` в waiting worker, затем **однократный** `controllerchange` или таймаут → `location.reload()` (без глобального `controllerchange` в `main.tsx`, чтобы не отменять загрузку документа в цикле).
