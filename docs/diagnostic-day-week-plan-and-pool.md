...
## 7) Анти-дубли недели (Mom Recipes): аудит и фикс (2025-02)

**Где механизм анти-повторов:**

- **Фронт:** `src/hooks/useGenerateWeeklyPlan.ts` — объект `accumulated` (chosenTitles, chosenBreakfastTitles, chosenBreakfastBases) передаётся в каждый запрос как `weekContext`. После каждого батча результаты мержатся в `accumulated`.
- **Edge:** `supabase/functions/deepseek-chat/index.ts` — для `type === "single_day"` из body берётся `weekContext`; если объект с `chosenTitles`, в промпт подставляется строка «Уже запланировано на неделю: … Не повторяй эти блюда». Доп. блок `buildVarietyRules(w)` (запрет овсянки дважды, разнообразие баз завтраков) подставляется в `{{varietyRules}}`.
- **Шаблон:** `supabase/functions/deepseek-chat/prompts.ts` — `SINGLE_DAY_PLAN_TEMPLATE` содержит `{{weekContext}}` и `{{varietyRules}}`.
- Клиентской проверки дублей по названиям после ответа нет; сравнение только в промпте (модель «не повторяй»).

**Почему не сработало:**

1. **Параллельность внутри батча:** Неделя генерируется батчами `[[0,1], [2,3], [4,5], [6]]`. В каждом батче дни уходят **одновременно** с одним и тем же `accumulated`. Например, дни 0 и 1 оба получают пустой контекст → модель не знает про другой день → возможны дубли Пн/Вт. Дни 2 и 3 получают контекст только Пн+Вт, но не друг друга → дубли Ср/Чт и т.д.
2. **Контекст не обнуляется** — он накапливается между батчами, но внутри батча один и тот же для всех запросов.

**Минимальный фикс:**

- Включена **последовательная** генерация дней внутри каждого батча за флагом: в DEV всегда `true`, в prod — при `VITE_SEQUENTIAL_WEEK_PLAN=true` (или `1`). При последовательном режиме дни батча вызываются по одному, после каждого результат мержится в `accumulated`, следующий день получает обновлённый контекст.
- Логи (только DEV): перед каждым single_day — dayName, planned_date, member_id, hasWeekContext, weekContextTitlesCount; после парсинга — titles и normalized keys по слотам; после применения недели — список всех normalized titles и список дублей (count + list). На Edge при DEBUG — лог weekContext (object/string/empty и число chosenTitles).

**Как проверить:** Сгенерировать неделю в DEV → в консоли в конце должен быть лог `[DEBUG] week apply done: ... duplicates count=0 list=(none)`. При дублях будет `duplicates count=N list=...`.
