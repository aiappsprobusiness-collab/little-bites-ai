...
**Синхронизация Client ↔ Edge (ranking):** один **`computeCompositeScore`**, общие **`buildAlignedRankSalt`** / **`rankJitterFromSeed`**, `rank_salt` в generate-plan и на клиенте; jitter **детерминирован** от `(salt, recipeId)`. Slot-fit на клиенте остаётся lite. Пороги candidate→trusted и SQL feedback **не трогались**.
