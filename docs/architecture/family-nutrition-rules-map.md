...
- **Rule:** Симпатии: мягкий сигнал в промптах чата (gating + анти-повтор по недавним title; см. **chat_recipe_generation.md** §6, **chatLikesSignal**). В **generate-plan** likes **не** участвуют в ранжировании пула. Предпочтения: семантические ограничения в промптах (напр. вегетарианское → без мяса/рыбы); в пуле и validateRecipe вегетарианство и предпочтения вида «без X» применяются как запреты.
- **Where:** Edge: `_shared/likesFavoring.ts` (shouldFavorLikes, buildLikesLine / buildLikesLineForProfile); `_shared/chatLikesSignal.ts` (recipe-path soft block + anti-repeat); deepseek-chat **index.ts** (сборка); **generateRecipeSystemPromptV3** без безусловных likes в [CONTEXT]. generate-plan **preferenceRules** — только allergies/dislikes. prompts STRICT_RULES. Client: derivePayloadFromContext; validateRecipe. DB: `members.likes`, `members.preferences`.
...
| **Likes**           | buildPrompt/derivePayloadFromContext | deepseek-chat: likesFavoring (buildLikesLine, shouldFavorLikes); prompt. generate-plan: **не** скорит likes | members.likes | ALLERGIES_AND_PLAN §7, PREFERENCES_*, PLAN_MENU_PROFILE |
...
| **Likes**     | **Данные:** `members.likes`. **Чат:** _shared/likesFavoring в системном промпте. **План (generate-plan):** не используется для ранжирования. Единого «словаря» для симпатий нет. |
...
- **Likes:** **Prompt (chat):** _shared/likesFavoring (buildLikesLine). **Plan:** generate-plan does not rank by likes. Client pool pick also does not score likes. Data source: `members.likes`.
...
- **Likes / preferences:** Изменение мягкого сигнала likes в **чате** — промпты (likesFavoring) и deepseek-chat index. **План:** likes не скорятся в generate-plan; при возврате скоринга по likes в план — менять `generate-plan/index.ts` и `PLAN_MENU_PROFILE_AND_RECIPE_SELECTION.md`. Изменение “preferences as strict” affects prompts (STRICT_RULES) and client validateRecipe (VEGETARIAN_BANNED, “не”/“без”). Обновлять и текст промпта, и валидацию при добавлении новой семантики предпочтений.
