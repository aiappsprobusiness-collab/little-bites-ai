...
|--------|---------|-----------|
| **under_12_curated_block** | `targetIsFamily === false`, профиль **child** (`members.type` или fallback см. ниже), числовой **age_months** (или **ageMonths**) ∈ [0, 11] | Ответ с текстом-подсказкой, `recipes: []`, `route: "under_12_curated_recipe_block"`, `reason_code: "under_12_curated_recipe_block"`. **Без LLM**, без сохранения рецепта, без retry. Подсказки по введению продуктов — во вкладке «Помощь маме»; проверенные блюда — **план прикорма** (пул рецептов). |
...
Текст поля **`message`** при **`route: "under_12_curated_recipe_block"`** задаётся константой **`UNDER_12_CURATED_RECIPE_BLOCK_MESSAGE`** в **`recipeGenerationRouting.ts`**: мягкое объяснение, что рецепты в чате не создаются автоматически (акцент на бережные и проверенные рекомендации); предложение выбрать блюда в **плане прикорма** и заглянуть в **«Помощь маме»** за подсказками по введению продуктов.
...
- **Клиент:** `src/utils/chatBlockedCheck.ts` — `checkChatRequestAgainstProfile`. Матч запроса: **`containsAnyTokenForAllergyInWords`** (prefix/suffix по словам, без ложных «пекан»/«рож» в «запеканка»/«творожный»).
...
Ответы redirect/irrelevant возвращают в теле JSON поля `route` (`assistant_topic` или `irrelevant`), при `assistant_topic` — `topicKey` и `topicTitle` (и при необходимости `topicShortTitle`) для перехода во вкладку «Помощь маме» по сценарию (`/sos?scenario=<topicKey>`). Тексты `message` для пользователя — константы **`CHAT_MESSAGE_ASSISTANT_REDIRECT`** и **`CHAT_MESSAGE_IRRELEVANT`** в **`supabase/functions/_shared/chatRecipeRoutingMessages.ts`** (двойной источник правды с клиентским `chatRouteFallback.ts`). На фронте такие сообщения отображаются карточкой системной подсказки (`SystemHintCard`), без кнопок рецепта (избранное, поделиться, в план). Клиент при сохранении в `chat_history` записывает в `meta` поля `systemHintType`, `topicKey`, `topicTitle`, `topicShortTitle`, чтобы после переключения вкладки/remount карточка и кнопка «Перейти в тему» восстанавливались с корректной навигацией в нужную тему. Для **`under_12_curated_recipe_block`** в `meta.systemHintType` сохраняется **`curated_under_12_recipe`** (кнопки «Открыть план» / «Помощь маме»).

**Быстрый путь в `ChatPage`:** **`getRedirectOrIrrelevantMeta`** вызывает тот же **`resolveRecipeChatIntent`**, что и Edge (`recipeChatIntent.ts`). Если маршрут не `recipe`, запрос к **`deepseek-chat` не отправляется** — сразу показываются `SystemHintCard` и сохранение в историю; пользователь не видит бабл ожидания с советами «как при генерации рецепта». Отдельного дублирования ключевых слов на клиенте нет — источник правды один с Edge.
...
- **maxCookingTime и servings:** передаются в теле запроса с клиента (если есть) и подставляются в промпт. **Клиент (чат):** для `type === "chat"` и `type === "recipe"` в `useDeepSeekAPI` в тело запроса добавляется **`servings`**, рассчитанное в **`src/utils/chatRecipeServings.ts`** (`resolveChatRecipeServings`: учёт семьи vs один профиль, без детей &lt;12 мес в подсчёте «едоков» для семьи, коэффициент для snack/lunch, clamp 1–8). **Edge (`deepseek-chat/index.ts`):** если `servings` не передан или невалиден, по умолчанию **servings = 1**.
- **Отображение порций и `servings_base` при сохранении с клиента:** в **`ParsedRecipe`** попадает **`servings`** из JSON (`servings`, `servings_base`, `servingsCount`), если модель их вернула. **`ChatRecipeCard`** показывает «Порции: N» как `recipe.servings`, иначе снова **`resolveChatRecipeServings`** (согласовано с запросом генерации). При сохранении через **`saveRecipesFromChat`** в **`createRecipe`** уходят **`servings_base` / `servings_recommended`**: приоритет у числа из распарсенного рецепта, иначе **`resolveChatRecipeServings`** на момент сохранения (`ChatPage`). Клиентский **`canonicalizeRecipePayload`** (`src/utils/recipeCanonical.ts`) включает эти поля в payload RPC `create_recipe_with_steps`.

---

## 9. Anti-duplicate и сохранение рецепта

- **Не повторять недавние рецепты (без ретраев):** Edge запрашивает за последние 14 дней `recipe_id` из `chat_history` по user_id и (в режиме «Семья») по storageMemberId, затем названия рецептов из `recipes`, нормализует их в `titleKey`.  
  Для **простых meal-only запросов** (например, «ужин», «на ужин») применяется mealType-scoped антидубликат: в промпт попадают до **5 последних названий этого же mealType** (`recentTitleKeysLine`: «Не повторяй недавние варианты: …»).  
  Для более конкретных/уникальных запросов (например, с явной кухней/блюдом) этот список в промпт не добавляется, чтобы не раздувать контекст и не мешать точному интенту пользователя.  
  На **Edge** на один HTTP-запрос — один вызов модели; ретраев «внутри» одного запроса нет. Лог `ANTI_DUPLICATE` остаётся диагностическим (`wasDuplicate`, `retried:false`).
  **Клиент (`ChatPage`):** при совпадении заголовка рецепта с последним сохранённым в сессии возможен **второй последовательный** запрос к `deepseek-chat` с тем же пользовательским сообщением; в тело передаётся **`isRetry: true`**. Запись в **`usage_events`** с `feature: "chat_recipe"` выполняется только если **`isRetry` не true** (одно списание на пользовательское действие «отправить», даже при двух вызовах API).
- **Лёгкий cooldown по технике (без отдельного ретрая):** для тех же простых meal-only запросов Edge дополнительно анализирует последние рецепты этого mealType и извлекает до 2–3 недавних техник (`запекание`, `тушение`, `жарка`, `варка`, `на пару`) по title+steps. Если найдено минимум 2 техники, в промпт добавляется короткая строка «избегай техник …, выбери другую». После генерации сервер повторно определяет технику итогового рецепта по title+steps и пишет диагностику попадания в cooldown в лог (`techniqueCooldownHit`), без дополнительного вызова модели.
- **Телеметрия anti-duplicate:** лог `ANTI_DUPLICATE_TELEMETRY` фиксирует `simpleMealOnlyQuery`, `mealTypeForPrompt`, число учтённых recent titles и факт применения/размер technique cooldown; лог `ANTI_DUPLICATE` дополнительно содержит `generatedTechnique`, `techniqueCooldownHit`, `techniqueCooldownList`.
- Тот же список recent title keys продолжает использоваться для **анти-доминирования likes** (см. §6).
...
- **parseRecipesFromChat** / **parseRecipesFromApiResponse** (`src/utils/parseChatRecipes.ts`): извлечение JSON рецепта из ответа (code block, regex, баланс скобок), нормализация ингредиентов и шагов, определение mealType по тексту, формирование ParseRecipesFromChatResult (recipes, displayText). В ParsedRecipe попадают **КБЖУ** с верхнего уровня или из `nutrition` (числа и строковые числа из JSON); поле **`nutrition_goals`** из ответа API/`recipes[]` (или из JSON); при наличии в JSON — **`servings`** (`servings` / `servings_base` / `servingsCount`). Текст `displayText` дополняется строками ккал/БЖУ, если они есть. Единое текстовое отображение количеств ингредиентов в карточке и списке покупок — **`shared/formatIngredientForUI.ts`** (dual: «шт./ложки = г/мл»; для сухих специй с каноном в мл — приоритет ч. л. в UI).
