...
## 8. Тип приёма пищи (mealType) и время готовки
...
- **useDeepSeekAPI** (`src/hooks/useDeepSeekAPI.tsx`): собирает контекст (buildGenerationContext, derivePayloadFromContext, buildPrompt), проверяет блокировку (checkChatRequestAgainstProfile), отправляет POST в `/functions/v1/deepseek-chat` с messages, memberData, type, targetIsFamily, memberId, allMembers, generationContextBlock, mealType, maxCookingTime и т.д., обрабатывает ответ (JSON с message/recipes/recipe_id или blocked), при стриминге — разбор SSE (если бы Edge отдавал stream; в текущей реализации Edge всегда возвращает JSON).
