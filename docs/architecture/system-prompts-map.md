...
**Recipe parsing rules:** Prompt rules that define output shape: **RECIPE_STRICT_JSON_CONTRACT**, **RECIPE_SYSTEM_RULES_V3**, **MEAL_SOUP_RULES**, **RULES_USER_INTENT**, **STRICT_RULES**, **SAFETY_RULES** in **prompts.ts**. **MEAL_SOUP_RULES**: супы только для lunch; для dinner/breakfast/snack супы не предлагать; для запроса на обед — только супы и аналоги (окрошка и т.п.). Validation and parsing: **recipeSchema.ts** (Zod), **domain/recipe_io** (sanitizeAndRepair, **chefAdviceQuality.ts**, enforceChefAdvice → `string | null`), **_shared/parsing** (validateRecipe, retryFixJson). **`chefAdvice`:** генерируется LLM, затем детерминированный пайплайн (sanitize → pool-unsafe → **prepareChefAdvicePipeline** → **isChefAdviceLowValue** / **hasForbiddenChefAdviceStart**); слабый или общий совет **не сохраняется** (`null`). Повторный вызов модели (quality retry) только если совет не прошёл gate. Диагностика: env **CHEF_ADVICE_DEBUG=true** — лог `CHEF_ADVICE_DEBUG` (raw/normalized/accepted/reason/advice_source). **Canonical `recipes.description`:** после санитизации ответа и при сохранении в БД задаётся **buildRecipeBenefitDescription** из **_shared/recipeBenefitDescription.ts** (детерминированно от `nutrition_goals` + seed, без профильного тона в тексте; не LLM и не recipeDescriptionComposer); модель может по-прежнему возвращать поле `description` в JSON для совместимости/эвристик. Payload for DB: **_shared/recipeCanonical.ts** (canonicalizeRecipePayload; без подстановки текста в `chef_advice`, если передан null). RPC: **create_recipe_with_steps**. Триггер **recipes_validate_not_empty** для chat_ai/week_ai/manual требует только непустой **description**; `chef_advice` и `advice` могут быть оба NULL.
