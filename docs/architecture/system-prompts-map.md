...
**Recipe parsing rules:** Prompt rules that define output shape: **RECIPE_STRICT_JSON_CONTRACT**, **RECIPE_SYSTEM_RULES_V3**, **CHEF_ADVICE_RULES** (блок в **prompts.ts** — один конкретный инсайт, ~140 символов, без вводных «Для более/Чтобы…»), **MEAL_SOUP_RULES**, **RULES_USER_INTENT**, **STRICT_RULES**, **SAFETY_RULES** in **prompts.ts**. **MEAL_SOUP_RULES**: супы только для lunch; для dinner/breakfast/snack супы не предлагать; для запроса на обед — только супы и аналоги (окрошка и т.п.). Validation and parsing: **recipeSchema.ts** (Zod), **domain/recipe_io** (sanitizeAndRepair, **chefAdviceQuality.ts**, enforceChefAdvice → `string | null`), **_shared/parsing** (validateRecipe, retryFixJson). **`chefAdvice`:** LLM → sanitize → **prepareChefAdvicePipeline** → **hasForbiddenChefAdviceStart** (пафос/шаблон в начале; **не** режет сочетания «Для более…»/«Чтобы…» сами по себе) → **isChefAdviceLowValue** (ровно одно предложение, длина ≤ **CHEF_ADVICE_MAX_LENGTH** 160; если текст начинается с «Для более»/«Для лучшего»/«Чтобы» — нужны одновременно **якорь к рецепту**, **конкретное действие** и **ощутимый эффект**, иначе `intro_start_without_concrete_bar`) → **enforceChefAdvice**; слабый совет → **`null`** (без детерминированных заглушек в hot path). Общие фразы без якоря к рецепту и без техники режутся в **isChefAdviceLowValue**. **Второй полный вызов модели из-за провала гейта `chef_advice` не выполняется** (один основной LLM-вызов на рецепт). Диагностика: **CHEF_ADVICE_DEBUG=true** — лог `CHEF_ADVICE_DEBUG` с полями `request_id`, `raw_chef_advice`, `normalized_chef_advice`, `accepted`, `rejection_reason`, `final_chef_advice`, `retry_skipped_due_to_advice_failure` (лог пишется после leak guard, финальное значение совпадает с БД/ответом). **`recipes.description` (чат + БД, один текст):** **pickCanonicalDescription** — после санитайзеров LLM-текст, если **passesDescriptionQualityGate** (1–2 предложения, длина ~38–210, штампы, нутритивный маркер, не дубль title) и нет **textContainsRequestContextLeak**; иначе fallback **buildRecipeBenefitDescription**. Payload RPC, `message` (JSON) и `recipes[0].description` совпадают; **после** `create_recipe_with_steps` описание **не** перезаписывается benefit-builder на Edge. Клиент **`createRecipe` с `source: 'chat_ai'`** не подменяет description на benefit после RPC (`useRecipes.tsx`). Диагностика описания: **CHAT_DESCRIPTION_DEBUG=true** — `request_id`, `raw_llm_description`, `llm_description_accepted`, `rejection_reason`, `final_description_source` (`llm` \| `deterministic_fallback`), `final_description`. **_shared/recipeCanonical.ts**, RPC **create_recipe_with_steps**. Триггер **recipes_validate_not_empty**: непустой **description**; `chef_advice` и `advice` могут быть NULL.
...
**Recipe JSON:** The model is asked to return a single JSON object (response_format: json_object for recipe path). Expected structure (from RECIPE_STRICT_JSON_CONTRACT / RECIPE_SYSTEM_RULES_V3): title, description (short, ≤210 in schema; LLM-first canonical via **pickCanonicalDescription** on Edge), ingredients [{ name, amount }] (max 10), steps (5–7, max 150 chars each), cookingTime, mealType (breakfast|lunch|dinner|snack), servings, chefAdvice (≤220 chars or null), nutrition (kcal_per_serving, protein_g_per_serving, fat_g_per_serving, carbs_g_per_serving, is_estimate: true).

**Validation:** **recipeSchema.ts** (Zod schema), **domain/recipe_io** (sanitizeAndRepair, **pickCanonicalDescription**, enforceChefAdvice), **_shared/parsing** (validateRecipe, retryFixJson, getRecipeOrFallback). Normalization of mealType, nutrition, ingredients (canonical amount/unit) in recipeSchema and parsing. After validation, **canonicalizeRecipePayload** (_shared/recipeCanonical) builds payload for **create_recipe_with_steps** RPC.

**Where validation happens:** deepseek-chat/index.ts after LLM response: parse response body, extract JSON, validateRecipe (parsing); затем санитайзеры, **pickCanonicalDescription**, **enforceChefAdvice**, сохранение. **Files:** deepseek-chat/recipeSchema.ts, deepseek-chat/domain/recipe_io/index.ts, deepseek-chat/domain/recipe_io/sanitizeAndRepair.ts, _shared/parsing/index.ts, _shared/recipeCanonical.ts.
