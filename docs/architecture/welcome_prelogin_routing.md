...
- **Не авторизован**, в URL нет токенов из письма: **нет** `hasSeenWelcome` (первый визит): если `VK_ROOT_REDIRECT_ENABLED` → `/vk` (приоритет над welcome, см. `docs/architecture/vk-funnel.md`). Иначе при `WELCOME_PRELOGIN_FROM_ROOT_ENABLED` → `/welcome`. Иначе → `/auth?mode=signup&…` (query с `/` сохраняется), `location.state.fromRootFirstVisit` → на `Auth` выставляется `hasSeenWelcome` и снимается `state`. **Повторный** заход → редирект на `/auth` (вкладка вход), `search` с `/` к `/auth` передаётся.
