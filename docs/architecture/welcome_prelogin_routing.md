...
### Root `/`
- **Авторизован**, после загрузки `members`: при **0** записей в `members` → редирект на `/profile?openCreateProfile=1&welcome=1` (создание первого ребёнка, как после письма подтверждения); при наличии членов семьи → `/meal-plan` (текущий app-home).
