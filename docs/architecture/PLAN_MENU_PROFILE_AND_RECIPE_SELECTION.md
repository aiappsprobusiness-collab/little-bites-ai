...
## 4.6 Разнообразие недели по «основе» блюда (primary base)
...
- Likes остаются мягким сигналом: рецепты с любимым продуктом могут попадать чаще, но штраф по базе не даёт одной основе забивать всю неделю.
...
- Пул: `fetchPoolCandidates` (источники: seed, starter, manual, week_ai, chat_ai). Для каждого слота: `pickFromPoolInMemory` с полным набором фильтров (exclude ids/titles, meal_type, обед = только супы, завтрак без супа, sanity, профиль: аллергии, dislikes, возраст при одном профиле, семейный ужин при member_id = null), затем скоринг: likes (favor/avoid), цели питания (`selected_goal`, дневное разнообразие — Stage 4.2), возраст/семья/soft-mode (Stage 4.3, без сужения пула в семейном режиме), недельное разнообразие по primary base при недельном плане.
