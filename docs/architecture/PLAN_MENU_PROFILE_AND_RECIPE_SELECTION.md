...
- **replace_slot (Edge):** для серверных сценариев — тот же «прикормовый» подбор (`pickInfantComplementaryFromPool`). **Экран плана** для прикорма (&lt;12 мес, Premium): замена ↻ идёт на **клиенте** — `pickRecipeFromPool` + `replaceSlotWithRecipe` с ролями слота (`infantSlotRole`: primary = завтрак, secondary = обед), без вызова Edge `replace_slot`. Для полдника/ужина в этом режиме замена на UI не предлагается (слоты скрыты).
...
- **Прикорм (&lt;12 мес), блок «Сегодня можно попробовать» (в БД `meal_type` = `INFANT_PLAN_SLOT_NEW_PRODUCT` = `breakfast`):** `evaluateInfantRecipeComplementaryRules`. **Старт (0 введённых):** одна строка продукта из `ALLOWED_START_PRODUCT_KEYS`. **После старта:** **ровно один** новый продукт (только знакомые комбинации — во втором блоке). Подписи: `getInfantPrimaryIntroducingLinesFromIngredientNames`. Подбор: `pickInfantNewRecipe` / `listInfantNewRecipeCandidates` (не смешивать с знакомым блоком).
- **Блок «Уже знакомое»** (`INFANT_PLAN_SLOT_FAMILIAR` = `lunch`): только ингредиенты из `introduced_product_keys` — `evaluateInfantSecondaryFamiliarOnly`; `pickInfantFamiliarRecipe` / `listInfantFamiliarRecipeCandidates`. На экране **два слота** рендерятся по наличию введённых продуктов, **не** по числу кандидатов в пуле (второй блок не скрывается из‑за пустого пула при непустом `introduced_product_keys`).
- **Нормализация в ключи:** `normalizeIngredientToProductKey` / `extractAllKeyProductKeysFromIngredients`; для кнопки «Добавить …» — `extractProductKeysForIntroduceClick`, `getInfantNovelProductKeysForIntroduce`.
- **Роль слота в пуле:** явный `infantSlotRole` в `pickRecipeFromPool` / `filterPoolCandidatesForSlot`; если не передан и `age_months < 12` — `resolveInfantSlotRoleForPool`: **только** для слотов `breakfast` / `lunch` — **завтрак = primary**, **обед = secondary**; для `snack` / `dinner` роль не задаётся (подбор по `meal_type` слота, как у 12+). Экран прикорма и `pickInfant*` передают нейтральный `mealType` + явную роль. **`useReplaceMealSlot.pickReplacementFromPool`** для `age_months < 12` и слотов breakfast/lunch использует ту же `filterPoolCandidatesForSlot`, что и основной пул (не режет кандидатов по breakfast/lunch в БД).
- **Режим возраста (UX):** `getInfantFeedingMode`: до 6 мес — `early_start` (плашка на плане: ранний старт по согласованию с врачом); с 6 мес — `standard`.
