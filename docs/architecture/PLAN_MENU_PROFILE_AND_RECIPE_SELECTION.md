...
- Полный выбор цели питания — в нижнем sheet; в hero только текущее значение цели (без отдельной подписи-заголовка).  
- В hero иерархия действий: **«Собрать день»** (primary) → **«Поделиться днём»** (outline) → **«Собрать неделю»** (текстовый tertiary teaser; у Free — 🔒 и метка Premium, клик → paywall / preview).  
- «Поделиться неделей» — только в меню «Ещё» для подписчиков.
