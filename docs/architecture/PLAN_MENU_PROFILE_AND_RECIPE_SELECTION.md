...
- В **run** и **upgrade** (и при **replace_slot**):
  - Если `member_id == null` (режим «Семья»): Edge сам подтягивает всех членов семьи из БД и вызывает `buildFamilyMemberDataForPlan(members)` → получает объединённые `allergies`, `dislikes`, `likes`, `preferences` и **без возрастного фильтра** (`type: "adult"` по смыслу для пула).
  - Если `member_id` задан: при необходимости Edge дополняет/перезаписывает `member_data` данными из `members` (allergies, likes, dislikes, age_months) для актуальности.
...
- **Клиент** (подбор из пула при «Подобрать рецепты», замена слота из пула, `useGenerateWeeklyPlan` при pool):  
  В `recipePool.ts`: `passesProfileFilter` использует `getAllergyTokens(memberData)` из `allergenTokens.ts` (тот же словарь алиасов, что и на Edge). Проверяется текст: **title, description, tags**. На клиенте запрос к `recipes` не подгружает `recipe_ingredients`, поэтому проверка аллергий по ингредиентам на клиенте **не выполняется** — только по title/description/tags. Полная проверка по ингредиентам есть только на Edge.
...
- Рецепт запрещён, если в объединённом тексте (title + description + ingredients на Edge; title + description + tags на клиенте) встречается **любая** подстрока из набора токенов.
...
Likes — **мягкий сигнал**: не запрет, а предпочтение. Цель — иногда включать рецепты с любимым ингредиентом/темой, но не перегружать ими план (например, не каждый день «любит ягоды»).

### 4.2 Реализация в generate-plan (Edge)

- Из `effectiveMemberData.likes` строятся токены: `preferenceRules.buildLikeTokens(memberData)`.
- Для каждого **дня** ведётся счётчик «уже был рецепт с like в этом дне» (`dayLikedRecipeCount`).
- Для каждого **слота** задаётся режим:
  - **favor** — предпочитать рецепты, в которых есть совпадение с like-токенами (им даётся бонус к скорингу);
  - **avoid** — слегка смещать выбор в сторону рецептов без like (штраф к скорингу за совпадение с like);
  - **neutral** — не учитывать likes в скоринге.

Правило переключения: если в текущем дне уже был хотя бы один рецепт с like **или** в одном из последних двух дней был такой рецепт (`recentLikedDayWindow`), для следующих слотов используется **avoid**, иначе — **favor**. Таким образом, фактически ограничивается «не более одного явного like-рецепта в день» и не чаще, чем примерно раз в 1–2 дня.

### 4.3 Скоринг (Edge)

- `scoreLikeSignal(recipe, likeTokens, mode)`: при `mode === "favor"` совпадение с like даёт плюс к баллу, при `mode === "avoid"` — минус. Учитываются title, description, recipe_ingredients.
- Итоговый выбор рецепта в слоте — по сумме: like-сигнал + recency + штраф за повтор по разнообразию (variety penalty).

### 4.4 Клиент

- В `recipePool.pickRecipeFromPool` **нет** скоринга по likes: после фильтров (аллергии, dislikes, возраст, слот, sanity) из отфильтрованных кандидатов выбирается случайный из топ-N. То есть на клиенте «любит» при подборе рецептов в план **не усиливает** выбор, но и не запрещает — рецепты с like просто не получают приоритета.

### 4.5 Целевая доля «любит ягоды» (25%)

В [PREFERENCES_BERRIES_RATIO.md](../decisions/PREFERENCES_BERRIES_RATIO.md) описана цель: при предпочтении «любит ягоды» — около 25% слотов с ягодными рецептами. В текущей реализации generate-plan используется описанная выше логика **favor/avoid** по дням и слотам (не более одного like в день, затем avoid), а не явный счётчик 25% по ягодам. То есть целевая доля в коде реализована как «мягкое ограничение частоты» через favor/avoid, а не как точный расчёт 25% по слотам.
...
- Пул: `fetchPoolCandidates` (источники: seed, starter, manual, week_ai, chat_ai). Для каждого слота: `pickFromPoolInMemory` с полным набором фильтров (exclude ids/titles, meal_type, обед = только супы, завтрак без супа, sanity, профиль: аллергии, dislikes, возраст при одном профиле, семейный ужин при member_id = null) и скорингом по likes (favor/avoid).
...
| **Любит** | Объединённый список всех членов | likes выбранного профиля | Только Edge: мягкий скоринг (favor/avoid), не более одного like-рецепта в день. Клиент — не усиливает выбор. |
