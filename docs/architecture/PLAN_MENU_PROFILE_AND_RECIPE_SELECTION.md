...
- **replace_slot, режим персонального пула:** в теле запроса опционально `replace_pool: "personal"` (клиент Плана: со **2-й** успешной автозамены того же слота за день для 12+ / «Семья»). Тогда сначала кандидаты только с `source IN ('manual','week_ai','chat_ai')` — те же фильтры профиля (`passesPreferenceFilters`, возраст, слот, обед=супы и т.д.) и тот же скоринг внутри среза; если подходящего нет — **fallback** на полный пул как при `default`. Прикорм и infant seed-only поле игнорируют.
...
- **Прикорм (&lt;12 мес), блок «Сегодня можно попробовать» (в БД `meal_type` = `INFANT_PLAN_SLOT_NEW_PRODUCT` = `breakfast`):** `evaluateInfantRecipeComplementaryRules` с объединением ключей из **ингредиентов** и **`title` + `description`** (`mergeCanonicalProductKeys` в `shared/infantComplementaryRules.ts`, алиасы — `shared/keyIngredientSignals.ts`, в т.ч. «желток» → `egg`). **Старт 4–6 мес (0 введённых):** ровно **один** ключ из `ALLOWED_START_PRODUCT_KEYS` (кабачок / брокколи / цветная капуста). **7–11 мес (0 введённых):** одна **новинка** вне стартовой тройки + опционально овощи из тройки (например кабачок + индейка); либо одно пюре только из тройки. **После отметки введённых:** **ровно один** новый продукт, остальные только из `introduced_product_keys`. Edge `cheap_zero` для прикорма не требует secondary-кандидатов, если введённых продуктов нет. Hero плана: подсказка про блок «Уже введённые продукты» (`INFANT_PLAN_HERO_INTRODUCED_PRODUCTS_HINT`). Подпись в UI плана над primary: **`getInfantPrimaryProductSummaryParts`** — первая строка «Новый продукт: …» (тот же акцент, что у прежней секции «Новый продукт»), вторая при необходимости — «Знакомый продукт: …» (вторичный стиль); `getInfantPrimaryProductSummaryLine` оставлен как компактная однострочная форма для совместимости. `getInfantPrimaryIntroducingLinesFromIngredientNames` — для других сценариев. Подбор: `pickInfantNewRecipe` / `listInfantNewRecipeCandidates` (не смешивать с знакомым блоком).
...
- **Режим возраста (UX):** `getInfantFeedingMode`: до 6 мес — `early_start`, с 6 мес — `standard` (классификатор в коде). **План (hero):** `getInfantPlanHeroNoticeKind` — **0–3 мес** `too_early`, **4–5 мес** `doctor`, **6+** без янтарного блока; основной абзац — `getInfantPlanHeroBodyParagraph` (до 4 мес — без «1–2 раза в день»).
...
- **Где задаётся:** `PoolExhaustedSheet` (после исчерпания пула); пустой день на `MealPlanPage` — «Сгенерировать в чате» / вторичное действие в чат с `firstEmptySlotId` для `mealType`; **пустой слот** в списке при частично заполненном дне — те же переходы в избранное и чат с prefill, что и в `PoolExhaustedSheet`, по `mealType` конкретного слота (`slot.id`).
