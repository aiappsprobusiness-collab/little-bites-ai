...
**Клиент:** вкладка «План» показывает упрощённый экран: заголовок «Прикорм на сегодня», дата, затем чип выбора профиля (`MemberSelectorButton` с `leadingEmoji` 👶 и `fitLabelWidth`, чтобы имя не обрезалось ellipsis), затем возрастные тексты; первая строка текста после чипа для всех возрастов: «В этом возрасте основное питание — грудное молоко или смесь.»; вторая строка — `infantComplementaryIntroSecondLineText` в `src/utils/infantComplementaryPlan.ts` (0–6 мес: прикорм вводится постепенно; 7–8: рацион постепенно расширяется; 9–11: рацион становится разнообразнее; далее в каждом варианте «обычно 1–2 раза в день»); «Сегодня мы подобрали подходящий вариант прикорма для вашего малыша.» без изменений; следующая строка — `infantComplementaryPickQualificationText` (0–6: мягкий и безопасный для начала прикорма; 7–8: мягкий и понятный на этапе расширения прикорма; 9–11: подходит для текущего этапа питания малыша); без кнопок «Собрать день/неделю», без списка покупок и «Отправить меню»; ссылки **«Помощь маме»** (`/sos`); под карточками — возрастная подсказка (`getInfantComplementaryAgeBandU12` + `infantComplementaryGuidanceExtraText` в том же файле): **0–6 мес** — «В начале прикорма достаточно одного нового блюда в день.»; **7–8 мес** — «Если малыш уже привык к прикорму, можно предложить ещё одно блюдо позже.»; **9–11 мес** — «Рацион становится разнообразнее, можно предлагать 1–2 блюда в день.»; затем общий текст про постепенный ввод продукта. **Карточки слотов:** **0–6 мес** — только «Основное блюдо» (блок «Второе блюдо» не показывается); **7–8 мес** — «Второе блюдо» только если в плане уже заполнен обед; **9–11 мес** — второй слот показывается, если в плане есть обед **или** в клиентском пуле есть кандидаты для `lunch` (только отображение; Edge generate-plan не меняется). Пустой слот без «Блюдо не выбрано»: текст «Мы подбираем подходящий вариант прикорма…» и лёгкий индикатор загрузки (не как ошибка). Если по тем же фильтрам, что у клиентского пула (`listFilteredPoolRecipesForPlanSlot` для слота breakfast), **нет ни одного** рецепта — один блок с текстом «Пока нет подходящих вариантов…» и ссылкой на `/sos`, **без** карточек слотов; там же выводится та же возрастная подсказка + текст про наблюдение за реакцией. Пока слот без `recipe_id` в развёрнутом плане, на клиенте последовательно добираются блюда из пула (`pickRecipeFromPool` + `replaceSlotWithRecipe`: исключения по неделе и уже подставленным в этом проходе; если строгий подбор вернул null — повтор без exclude). Для `<12 мес` при наличии `members.introduced_product_keys` после базовых фильтров применяется **soft ranking** по ключевым ингредиентам (`recipe_ingredients`) и возрастной группе 4–6/7–8/9–11, без жёсткого отсева; если список введённых продуктов пуст — поведение как раньше. На заполненной карточке есть действие **«Уже пробовали»** (нормализует продукт в `product_key` и сохраняет в `members.introduced_product_keys`), а в hero — кнопка редактирования списка введённых продуктов (необязательный шаг). При смене выбранного дня сбрасывается `replacingSlotKey`, чтобы не «залипала» замена после перехода на другой день. Смена варианта у заполненной карточки — только иконка ↻ (Premium: `replaceMealSlotAuto`, как в режиме 12+). В dev / `?debugPool=1` — лог `[infant_plan_fill]` (число кандидатов после фильтра, `usedRecipeIds`, `day_key`, `age_months`). Логика Edge generate-plan не меняется. Чипы целей плана и `nutrition_goals` в превью скрыты.
...
**Профиль ребёнка &lt; 12 мес (не «Семья»):** отдельная ветка UI без hero-CTA «Собрать день/неделю», без «Собрать список продуктов» и «Отправить меню» (в т.ч. баннер после онбординга и пункт «Отправить меню на неделю» в меню «Ещё»); заголовок «Прикорм на сегодня», hero-тексты и число слотов — по группам 0–6 / 7–8 / 9–11 мес (см. §5.4, `infantComplementaryPlan.ts`); пустой слот — «Мы подбираем…» до клиентского добора из пула; при нуле кандидатов в пуле (breakfast) — блок «Пока нет подходящих вариантов…» + `/sos`; смена варианта — только ↻ на карточке; для Premium в infant-flow действует лимит **до 5 автозамен на слот в день**; при исчерпании лимита или кандидатов показывается infant-specific fallback с CTA **«Показать подходившие варианты»** (без CTA «Подобрать в чате»); у Free при попытке замены остаётся текущий paywall-flow; добавлены действия **«Уже пробовали»** на карточке и **«Редактировать введённые продукты»** в hero; **«Помощь маме»** (`/sos`) остаётся без изменений и может использоваться как дополнительная точка ведения; чипы целей скрыты.
