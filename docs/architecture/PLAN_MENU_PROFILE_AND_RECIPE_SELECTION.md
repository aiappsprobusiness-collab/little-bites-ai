...
- **Клиент** (страница План, `MealPlanPage`) формирует `memberDataForPlan` и при вызовах Edge передаёт в теле запроса:
  ...
  - `member_data` — объект с полями `allergies`, `likes`, `dislikes`, при одном профиле ещё `age_months`, `type`, при семье — объединённые списки и `type: "family"`.
...
- **Один профиль** (выбран конкретный член семьи или Free с одним профилем):
  - Берутся `allergies`, `likes`, `dislikes`, `age_months`, `type` выбранного профиля.
...
- **Прикорм (&lt;12 мес), только UI:** `filterPoolCandidatesForSlot`, `listFilteredPoolRecipesForPlanSlot` и `pickRecipeFromPool` в `recipePool.ts` — единая цепочка фильтров для подсчёта кандидатов и смены варианта на экране; **не** подменяют Edge.
