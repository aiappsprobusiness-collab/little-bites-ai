...
- Кнопки генерации не дублируются: в hero — «Заполнить день», «Заполнить всю неделю», «Очистить»; при полностью пустом дне в empty state — «Собрать день» и вторичный переход в чат (prefill); при частично заполненном дне пустой слот в списке — «В Избранном» и «В чате» (как после исчерпания пула), строка слота: чип приёма пищи + «Не выбран», затем «Подобрать рецепт:».
