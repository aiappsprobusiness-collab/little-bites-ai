...
- **Scope `member_id` (критично):** в **`mealPlanMemberIdForShoppingSync`** был **`?? null`**, из‑за чего **`undefined`** (Free «Семья», `members` ещё пустой — как на MealPlanPage) превращался в **`null`**. RecipePage смотрел на кэш **только семейной строки** (`member_id IS NULL`), а план до загрузки детей запрашивался **без фильтра по member_id** (все строки) → разные слоты/`servings` в `find()`, плюс гонка с PATCH. Исправление: **`undefined` не коэрцировать в `null`**; в **`useMealPlans`** при `memberId === undefined` запрос **выключен** (`scopeReady`), отдельный ключ **`__pending_plan_member_scope__`**; явный режим **`allMemberRows`** для FamilyDashboard. Пока scope не готов — RecipePage ждёт (`planMemberScopePending`), на плане — скелетон (`planMemberScopePending`).
