...
- **Импорт:** `scripts/import-infant-seed.ts` (запуск через `npm run seed:infant:import` или `npx tsx --tsconfig tsconfig.scripts.json scripts/import-infant-seed.ts`) — **service role**, не `create_recipe_with_steps` (там `user_id` = `auth.uid()`). При вставке ингредиентов заполняется канон (`canonical_amount` / `canonical_unit`), если его можно вывести из JSON или из `amount`/`unit`/`display_text` (см. `shared/ingredientCanonicalResolve.ts`).
