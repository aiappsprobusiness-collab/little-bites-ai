...
- Подбор превью: reuse `vk-preview-plan` (`buildVkPreviewDayPlan`).
- **Финал:** текст с меню на день (4 приёма) + блоки ценности и оффера; **одна** inline-кнопка **«Открыть приложение»** → `/auth` с параметрами атрибуции (см. ниже). Кнопок по слотам рецептов и «Посмотреть ещё рецепты» в финале **нет** (повтор сценария — **`/start`**; легаси `again` / `restart` в коде по-прежнему обрабатываются, если пользователь жмёт старую кнопку).
...
## Финальная ссылка на `/auth` и аналитика
...
| Параметр | Назначение |
|----------|------------|
| `mode=signup` | Открыть форму регистрации (email + пароль). |
...
Реализация URL: `buildTelegramOnboardingFinalAuthUrl` в `supabase/functions/telegram-onboarding/cta.ts`. На клиенте атрибуция из query подхватывается при открытии `/auth` (`AuthPage` → `captureAttributionFromLocationOnce`) и далее попадает в события `usage_events` согласно `src/utils/usageEvents.ts`.
...
4. Нажать **«Открыть приложение»** → `/auth?mode=signup&entry_point=telegram&utm_source=…&utm_medium=…&utm_content=…` (значения см. таблицу выше).
...
| 2026-04 | Финал: одна кнопка «Открыть приложение», `/auth` с `mode=signup` + UTM/entry_point для аналитики без данных анкеты; текст финала с блоками ценности. |
