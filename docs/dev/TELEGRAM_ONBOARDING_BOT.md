...
- **Повторный заход (после успешной первой генерации):** при **`/start`** сразу сообщение с коротким CTA + **«Открыть приложение»** — **без** опроса и без вызова `vk-preview-plan`. Команда `again` / `restart` (легаси) по-прежнему ведут в анкету; после «Далее →» на дизлайках снова короткий CTA, не полное меню. Флаг `menu_example_delivered` в БД сохраняется.
...
- Отдельной кнопки «Заново» на промежуточных шагах нет. **Сброс сценария:** команда **`/start`**. Если пример меню уже был показан, **`/start`** сразу даёт короткий CTA без опроса (легаси `again`/`restart` всё ещё могут вести в анкету).
...
| Дата | Изменение |
|------|-----------|
| 2026-04 | Повторный CTA: переносы строк, «мамский» тон (MSG_REPEAT_AFTER_MENU_EXAMPLE в `orchestrate.ts`). |
