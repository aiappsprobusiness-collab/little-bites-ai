...
- Финальное сообщение: заголовок + **4 строки приёмов** (завтрак / обед / ужин / перекус) + одна CTA-строка без URL в тексте. Inline-кнопки: по слотам «Завтрак» … «Перекус» → **`/recipe/teaser/:id`** (публичный тизер карточки + размытые шаги; не `/recipe/:id`, т.к. тот маршрут защищён логином), затем «Зарегистрироваться» (`/auth`, `entry_point=telegram`) и «Посмотреть ещё рецепты» (`again` / как `/start`).
- На шагах с чипами кнопка **«Нет»** (`*:clear`) сразу переводит к следующему вопросу (и очищает выбор в этом шаге).
