...
- **Free, одна аллергия:** подсказка под полем и тост при срабатывании лимита задаются в UI (`FREE_ALLERGY_SINGLE_HINT_CREATE`, копирайт paywall — `FREE_ALLERGY_PAYWALL_MESSAGE` в `friendlyLimitCopy.ts`).
