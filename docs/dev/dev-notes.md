...
- **Клиент:** `requestPasswordReset` → `resetPasswordForEmail` с `redirectTo: <origin>/auth/reset-password`. **`isRecoverySession`** = `recoveryFromAuthEvent` **или** JWT с `amr.recovery` (нельзя требовать `session !== null` только для флага — до `getSession` сессия ещё null). Нельзя в `getSession` сбрасывать recovery, если JWT пока без `amr.recovery`, пока активен сброс (это затирало `PASSWORD_RECOVERY`). На `/auth/reset-password` при токенах в URL показывается ожидание сессии, а не мгновенный «срок ссылки истёк». Форма — `updateUser({ password })`. Старые письма с `/auth/callback`: `AuthCallbackPage` ведёт на `/auth/reset-password`.
