...
- **`AuthCallbackPage`**, **`RootRedirect` (`/`)**, **`AppPreloginScreen`**, **`LandingOnboardingScreen`**, редирект с **`/meal-plan`** при 0 профилях: если у пользователя нет записей в `members`, целевой путь — **`/profile/child/new?welcome=1`** (`PROFILE_FIRST_CHILD_ONBOARDING` в `src/utils/profileCreateRoutes.ts`). Legacy deeplink `/profile?openCreateProfile=1` на `ProfilePage` редиректит на тот же full-page create. Параметр `welcome=1` — приветственный блок на странице `ChildProfileEditPage` (не в шторке).
- **Создание и редактирование профиля** — один UI: `ChildProfileEditPage` (`/profile/child/new` и `/profile/child/:id`). Карточки «Пищевые особенности», ghost-блоки «Настроить (Premium)» для likes/dislikes на Free. `ProfileEditSheet` для create **не используется**; «Добавить профиль» на вкладке Профиль → `navigate` на `/profile/child/new`. Онбординг семьи в чате → `buildProfileChildCreateUrl({ returnPath: '/chat' })`. Paywall `add_child_limit` на `/profile/child/new` **не** показывается сразу после сохранения первого профиля (`shouldEnforceNewProfileMemberLimit` + `savedSuccessfullyRef`).
- **Глобальный paywall** (`UnifiedPaywall` / `LegacyPaywall` в `App.tsx`): слой `z-[100]`, чтобы окно подписки было **поверх** нижних sheet/диалогов (`z-50`).
- **Free, одна аллергия (создание профиля):** до первой записи — подсказка `FREE_ALLERGY_SINGLE_HINT_CREATE`. После добавления одной аллергии поле ввода **readOnly** (`TagListEditor`), текст `FREE_ALLERGY_LIMIT_REACHED_HINT` и ссылка `FREE_ALLERGY_UPSELL_LINK_LABEL` (`FreeAllergyUpsellHint`). Paywall с причиной `onboarding_second_allergy_free` открывается **только по клику на ссылку**, не при Enter/запятой. Хелперы: `src/utils/freeAllergyProfileUi.ts`. Формы: `ProfileEditSheet`, `AddChildForm`. Закрытие sheet не блокируется paywall; при закрытии sheet paywall сбрасывается.
- **Free, редактирование существующего профиля:** `ChildProfileEditPage` — `allergies_locked`, `FREE_ALLERGY_PAYWALL_MESSAGE`, disabled input при лимите (без изменений).
