...
- **Клиент:** `requestPasswordReset` → `resetPasswordForEmail` с `redirectTo: <origin>/auth/reset-password`. Доступ в основное приложение блокируется, пока не введён новый пароль: **`isRecoverySession`** = JWT с `amr.recovery` **или** событие `PASSWORD_RECOVERY` (его нельзя отбрасывать до `getSession` — иначе пользователь попадал в приложение сразу после ссылки из письма) **или** флаг `sessionStorage` после reload. Форма — **`/auth/reset-password`** (`updateUser({ password })`). Старые письма с `/auth/callback`: `AuthCallbackPage` по `type=recovery` ведёт на `/auth/reset-password`.
