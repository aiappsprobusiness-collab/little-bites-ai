...
**Первый профиль ребёнка (онбординг с Профиля):** после сохранения первого ребёнка — сразу переход на вкладку «План» (`/meal-plan?memberId=…&date=…`), без полноэкранного оверлея на Профиле. На Плане показываются привычный layout, скелетон приёмов и строка статуса подбора; заполнение дня (`startFillDay` в `planFill.ts`) уходит в фон после редиректа. На **Профиле** после сохранения нужно подавить повторное автооткрытие шторки создания ребёнка, пока контекст семьи ещё отдаёт `members.length === 0` (иначе эффект онбординга открывает шторку второй раз поверх перехода на План): **`suppressEmptyFamilyAutoOpenRef`** выставляется в `handleMemberCreated` при **любом** успешном создании; редирект на План при meta **`wasEmptyFamilyOnboarding`** — её источник в **`ProfileEditSheet`**: снимок **`members.length === 0` до `await createMember`** (не проп `skipFill`: после insert invalidate родитель может уже иметь `members.length >= 1`, и проп стал бы ложным). Флаг suppress не сбрасываем при каждом `members.length > 0`, только при переходе «были члены → стало 0», чтобы не снимать защиту в гонке с refetch. Отдельно: эффект deeplink **`?openCreateProfile=1`** (после письма) **тоже** должен уважать `suppress` и при подавлении **чистить query** — иначе при гонке `setSearchParams` эффект снова вызывал `setShowMemberSheet(true)`; см. `src/utils/profileSheetDeepLink.ts` + тесты.
