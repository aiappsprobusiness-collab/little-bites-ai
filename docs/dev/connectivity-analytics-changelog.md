...
## 2026-05-05 — UX: тексты для мам (экран до входа в App)

Заголовки в `ConnectivityGateScreen` и тексты в `checkAppConnectivity` (`connectivityUserMessages`) переписаны теплее и без «сухого» техно-языка; кнопка: **«Попробовать снова»**. Экспорт `connectivityUserMessages` — для тестов и единого источника строк.
