...
### 3.3. Блок «Совет от шефа» в карточке чата (UI)

- **Не прикорм** (`max_age_months ≥ 12` или без infant-диапазона): Free и Premium показывают подсказку в **одном оливковом блоке** (`recipeChefAdviceCard` в `RecipeCard`). На Free в карточку попадает поле `advice` (короткий совет) или fallback `chef_advice`; заголовок — «Совет от шефа».
- **Прикорм:** заголовок «Подсказка для мамы», нейтральный фон (`recipeMiniAdviceCard`), без «шефского» паттерна.
