...
**Curated infant seed (4–6 и 7–8 мес):** импорт и идемпотентность — `docs/dev/infant-seed-import.md` (`source = seed`, `trust_level = trusted`, возраст по `min_age_months` / `max_age_months`).
