...
- **Edge `vk-preview-plan` (`pickDbSlots` превью дня для VK/Telegram onboarding):**
  - Сначала тот же приоритет, что выше: для слота **lunch** только супы (`is_soup` / эвристика soup из заголовка).
  - Если после фильтров по возрасту, аллергиям и dislikes **ни один суп не остался**, выполняется **второй проход**: в слот **lunch** подбирается любое блюдо из пула с resolved `meal_type` = lunch (без требования супа). Это нужно, чтобы в превью показывались **реальные названия из каталога**, а не заглушки/mock, когда супы отсекаются ограничениями. На автозаполнение плана в приложении это **не распространяется** (`generate-plan` без изменений).
